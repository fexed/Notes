...
\paragraph{Learning Timing} \textbf{Eager}: analyze data and construct an explicit hypothesis\\
\textbf{Lazy}: store tr data and wait test data point, then construct an ad hoc hypothesis.
\paragraph{K-NN}
\paragraph{Voronoi Diagram} Each cell scosnist of point closer to $x$ than any other patterns. The segments are all points in plan equidistanto to two patterns. It is implicitly used by K-NN.
\paragraph{Artificial Neuron} Input from external source or other units, with weights $w$ as free parameters: can be modified by learning. The unit $i$ computes $f(\sum_j w_{ij}x_j)$ with $w_{ij}$ the weight from input $j$ to unit $i$. $f$ is called \textbf{activation function}: linear, threshold or logistic (sigmoid).
\paragraph{Perceptron} A neuron that uses a threshold as activation function. Can be composed and connected to build a network.
...
\paragraph{Learning for one unit model} \begin{enumerate}
	...
	Minimize number of misclassified patterns %TODO
	\begin{enumerate}
		\item initialize weights
		\item pick learning rate $\eta$ (between 0 and 1)
		\item until stopping condition (es weights don't change)
	\end{enumerate}
	For each training pattern $(x, d)$ compute output activation $out = sign(w^T x)$. If $out = d$ don't change weights, if $out \neq d$ update weights $w_{new} = w + \eta\cdot d\cdot x$ adding $+\eta x$ if $wx\leq 0$ and $d = +1$ or $-\eta x$ if $wx > 0$ and $d = -1$. Different form $w_{new} = w + \frac{1}{2}\cdot\eta\cdot (d-out)\cdot x$
\end{enumerate}
\paragraph{Perceptron convergence theorem}
\subparagraph{Preliminaries}
\subparagraph{Proof}
\paragraph{Activation functions}
\paragraph{Excercise} Compute the gradient
\paragraph{Gradient descent algorithm}
\paragraph{Neural Network} In a MLP architecture: units connected by wighted links, organized in layers:
\begin{list}{}{}
	\item input layer, source of the input $x$
	\item hidden layer, projects onto another hidden or an output layer
\end{list}
