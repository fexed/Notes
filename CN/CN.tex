...
Quindi si accettano valori come \texttt{nnz}(A) $= \left\{ \begin{array}{c}O(n) \\ O(n\cdot log^p n) \end{array} \right. $ ma principalmente \texttt{nnz}(A) $= O(n)$
\pagebreak
...
Dunque $$Ax = b \Leftrightarrow \begin{array}{c}
x = Px + q \\
P = M^{-1}N\\q = M^{-1}b\\
A = M - N
\end{array}$$
\paragraph{Perché} Questo perché per risolvere $x = Px + q$ posso usare un metodo composto in questo modo $\left\{ \begin{array}{c}
x^{(0)} \in R^n\\
x^{(k + 1)} = Px^{(k)} + q
\end{array} \right.$
...
\begin{list}{}{}
	\item $\left\{\begin{array}{c} x^{(0)} \in R^n\\x^{(k+1)} = Px^{(k)} + q \end{array}\right. $ che solitamente lo si implementa come $\left\{\begin{array}{c} x^{(0)} \in R^n\\Mx^{(k+1)} = Nx^{(k)} + b \end{array}\right. $ 
\end{list}
