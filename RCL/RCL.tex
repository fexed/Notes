...
Per i pacchetti IP frammentati, i \textbf{messaggi ICMP sono relativi solo al frammento 0}. Inoltre, i \textbf{messaggi ICMP non sono \textit{mai} inviati in risposta a pacchetti IP con indirizzo mittente che non rappresenta un host unico} (\texttt{0.0.0.0}, \texttt{127.0.0.1}, broadcast\ldots).\\
...
\columnbreak
\paragraph{VLAN} Le \textbf{Virtual Local Area Network}, VLAN, \textbf{consentono di disperdere fisicamente le varie porzioni di rete a piacere senza perdere l'identità di gruppo}. Si tratta di un concetto prettamente a livello data-link, quindi riguarda i frame trasmessi. Non è implementata negli host, che sono membri, ma è \textbf{implementata negli switch}.\\\\
