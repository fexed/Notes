...
\begin{list}{}{}
	\item[1.] $c_j$ is a unit-vector
	\item All the columns are unit vectors
	\item[2.] We can decompose $U$ as sum of rank-1 matrices
\end{list}
$$U=\sum_{k=0}^{d-1}|c_k\rangle\langle k| = |c_0\rangle\langle 0| + |c_1\rangle\langle 1|$$
...
$$\frac{1}{2}(\langle c_k|+\langle c_j|)(|c_k\rangle + |c_j\rangle) =$$
\begin{center}
...
We can also do with relative phases, for example $i|j\rangle$ $$U\left(\frac{|k\rangle + i|j\rangle}{\sqrt{2}}\right) =$$
...
\end{list}
$$(U^H)_{ij} = u^*_{ji}$$
And $U^H = U^* =U ^+$, $U^H$ is the conjugate transpose. Diagonal conjugate, invert triangles and transpose elements.\begin{center}
	\includegraphics[scale=0.5]{13.png}
\end{center}
\paragraph{Spectral Theorem} There are classes of matrices which have an orthonormal basis of eigenvectors, i.e. are diagonalizable.\\
...
$A = A^H$ hermitian matrices, $AA^H = A^HA$\\
\textbf{Unitary matrices are normal matrices}.\\
Spectral theorem: a matrix $A$ is normal $\Leftrightarrow$ $\exists\:U$ unitary and $D$ diagonal $|\:A=UDU^H$, meaning that $Au_i = \lambda_iu_i\:\forall\:i=1,\ldots,n$\\
I.e. the columns of $U$ are an orthonormal basis of eigenvectors and $\lambda_i$ are the corresponding eigenvalues.
\begin{center}
...
\end{center}
\paragraph{Degrees of freedom} With 1 qubit $\alpha|0\rangle+\beta|1\rangle$ normalized so the constraint $|\alpha|^2+|\beta|^2 = 1$ and another constraint: we consider the surface of the Bloch sphere of size $2$, so $4-2 = 2$ degrees of freedom.\\
...
\paragraph{Inner product of tensor products} %TODO
\end{document}