...
For example, given map($f$) $\cdot$ map($g$), meaning $x\mapsto f(x)\mapsto g(f(x))$, this is $\equiv$ map$(f(g))$ (meaning Map(Comp($f$, $g$))).\\
In time terms, suppose that over a single item I take $t_f$ to compute $f$ and $t_g$ to compute $g$. To execute the map, I have to setup a set of parallel activities, split $x$, operate a map over each partition, merge each partition and again split, compute the second map over each partition and merge again. In shared memory split and merge is not significant, but it's still overhead payed. If I just split a single time, computing the map of the composition of the two maps, we have $k\cdot t_f\cdot t_g$, obviously larger than the single map but here the overhead has a smaller impact over the overall computation.
...
\begin{list}{}{}
	...
	\item If one TaskTracker is slow, it can delay the whole MapReduce job, because \textbf{everything can end up waiting for the slowest task}
\end{list}
\section{Apache Spark}
One alternative to HDFS and MapReduce.\\
Apache Spark is a \textbf{unified analytics engine for large-scale data processing}. It provides high-level APIs in many languages and an optimized engine that supports general execution graphs. Spark \textbf{performs in-memory cluster computing}, increasing the processing speed of an application, also supporting SQL queries, machine learning and graph algorithms.
\paragraph{Basics} Every application consists of a \textbf{driver program} which \textbf{runs the main function} and \textbf{executes various parallel operations on a cluster}.\\
The main abstraction provided is a \textbf{RDD} (Resilient Distributed Dataset):
\begin{list}{}{}
	\item \textbf{Collection of elements partitioned across the cluster} that can be operated in parallel
	\item \textbf{RDD can be persisted in memory}, allowing efficient reuse across applications
	\item \textbf{RDD automatically recovers from node failures}
\end{list}
The second main abstraction is \textbf{shared variables}
\begin{list}{}{}
	\item when a parallel function is being run as a set of tasks on different nodes, Sparks \textbf{ships a copy of each variable used in the function to each task}
	\item a \textbf{variable may need to be shared across tasks}, \textbf{or between tasks and the driver} program.
	\item Spark supports two types of shared variables:
	\begin{list}{}{}
		\item \textbf{Broadcast}, used to cache a value in memory on all nodes
		\item \textbf{Accumulators}, only added to, such as counters and sums
	\end{list}
\end{list}
\paragraph{RDDs} Fundamental data structure of Spark
\begin{center}
	\includegraphics[scale=0.75]{39.png}
\end{center}
\begin{list}{}{}
	\item \textbf{Immutable distributed collection of objects}
	\item \textbf{Each dataset} inside an RDD is \textbf{divided in logical partitions}, each \textbf{may be computed on different nodes} of a cluster
	\item An RDD can contain any type of Python, Java or Scala objects, including user-defined classes
\end{list}
Formally, an RDD is a \textbf{read-only partitioned collection of records}. RDDs can be created through deterministic operations on either data on stable storage or other RDDs, and is a \textbf{fault-tolerant collection} of elements that can be operated on in parallel.\\
There are two ways of creating an RDD:
\begin{list}{}{}
	\item \textbf{Parallelizing an existing collection} in the driver program
	\item \textbf{Referencing a dataset} in an external storage system that offers an Hadoop Input Format
\end{list}
\paragraph{Operations} There are two supported types of operations:
\begin{list}{}{}
	\item \textbf{Transformations} create a new dataset from an existing one (e.g. map)
	\item \textbf{Actions} return a value to the driver program after running some computation on the dataset (e.g. reduce)
\end{list}
This distinction is used to separate \textbf{eager} and \textbf{lazy evaluation} of programs:
\begin{list}{}{}
	\item \textbf{Eager evaluation} is when an expression is \textbf{evaluated as soon as its bound} to a variable
...
\end{list}
\section{Actor Model}
It provides a simple and intuitive programming model for building concurrent and distributed systems. It \textbf{emphasizes message passing and actor isolation}, which helps in improving the modularity and scalability of the system.
\paragraph{Actor} Computational entity that, in response to a message it receives, can concurrently:
\begin{list}{}{}
	\item Send a \textbf{finite number} of messages to other Actors
	\item Create a \textbf{finite number} of new Actors
	\item \textbf{Designate the behavior to be used for the next message} it receives
\end{list}
These are concurrent actions, so \textbf{no assumed order}. This makes this model significantly different from BSP. Decoupling sender from communications is a fundamental advancement, enabling asynchronous communications.
\paragraph{Connections} An Actor can only communicate with the Actors to which it is connected. Connections can be implemented in different ways.
\paragraph{Inherently Concurrent} Actor model is characterized by inherent concurrency of computation:
\begin{list}{}{}
	\item Withing and among Actors
	\item By the dynamic creation of Actors
	\item By the inclusion of Actor addresses in messages (allowing to send responses)
	\item Interaction only through a direct asynchronous message passing with no restriction on the arrival order
\end{list}
\paragraph{Modularity} Modularity comes from the one-way asynchronous communication. Once a message has been sent, it becomes \textbf{responsibility of the receiver}. Messages are decoupled from the sender, delivedere by the system on a best-effort basis.\\
\textbf{Sequential ordering is a special case derived from concurrent computation}.
\paragraph{Indeterminacy} The Actor model is indeterminate because \textbf{the order in which messages are processed by actors is not fixed}. When multiple messages are waiting, the order in which they are processed is determined by the Actor's scheduling policy. This means that the behavior of an Actor may depend on the order of processing, and thus be unpredictable.\\
The only guarantees are that \textbf{messages will be delivered} and that \textbf{processing happens atomically within an Actor}, avoiding race conditions and other issues.
\paragraph{Quasi-Commutativity} Meaning that \textbf{the order in which messages are sent between actors does not affect the final outcome of the system}. This means that Actor model systems are resilient to message delays, losses and reordering.\\
This, however, is not a guarantee that the system will behave correctly in all circumstances.
\paragraph{Applications} The Actor model can be used to build
\begin{list}{}{}
	\item \textbf{Fault-Tolerant Systems}, allowing Actors to be restarted or moved to another machine in case of failure
	\item \textbf{Scalable Systems}, allowing Actors to be dynamically created and distributed across machines
	\item \textbf{Aynchronous Systems}, allowing one-way asynchronous communication between Actors, decoupling messages and eliminating the need for complex synchronization mechanisms
	\item \textbf{Resilient Systems}, providing a reliable message delivery mechanism that ensures the atomic delivery
\end{list}
\paragraph{CAP Theorem} Also named \textbf{Brewer's Theorem}, it states that \textbf{it is impossible for a distributed data store to simultaneously provide more than two of the following}:
\begin{list}{}{}
	\item \textbf{Consistency}: all nodes should see the same data at the same time
	\item \textbf{Availability}: node failures do not prevent survivors from continuing to operate
	\item \textbf{Partition tolerance}: system continues to operate despite network partitions
\end{list}
When a network partition failure happens, we may \textbf{cancel the operation} (decreasing availability but ensuring consistency) or \textbf{proceed with the operation} (providing availability but risking inconsistency).\\
The CAP theorem \textbf{describes the trade-offs that are involved in distributed systems}. It is \textbf{essential in making decisions} about the future of distributed system design, and \textbf{misunderstandings lead to erroneous or inappropriate design choices}. So the theorem basically says "only pick two". But \textbf{partition tolerance is mandatory in distributed systems}, and \textbf{consistency and availability cannot be achieved at the same time} in larger-scale distributed systems. So if there's a partition, \textbf{does the system give up availability or consistency}? CAP only prohibits the perfect availability and consistency in the presence of partitions.\\
Consistency or availability is not a binary decision: with no consistency we're not always inconsistent, and with no availability we are not totally unavailable. This enables the concepts of \textbf{best-effort availability/consistency}.
\subparagraph{Types of Consistency}
\begin{list}{}{}
	\item \textbf{Strong Consistency}: after an update, any subsequent access will return the same updated value
	\item \textbf{Weak Consistency}: not guaranteed that subsequent accesses will return the updated value
	\item \textbf{Eventual Consistency}, a specific form of weak consistency: it is guaranteed that if no new updates are made to an object, eventually all accesses will return the last updated value. Few variations:
	\begin{list}{}{}
		\item \textbf{Causal Consistency}: processes with causal relationships will see consistent data
		\item \textbf{Read-your-write Consistency}: a process always accesses the data item after its update operation, and never sees the old value
		\item \textbf{Session Consistency}: as long as the session exists, the system guarantees the read-your-write consistency
		\item \textbf{Monotonic Read Consistency}: if a process has seen a particular value, any subsequent processes will never return any previous value
		\item \textbf{Monotonic Write Consistency}: the system guarantees to serialize the writes by the same process
	\end{list}
\end{list}
In practice a number of these approaches can be combined, with Monotonic Reads and Read-your-write the most desirable.
\paragraph{PACELC Theorem} Extending CAP, it states that \textbf{in case of network partitioning} (P) in a distributed system \textbf{one has to choose between Availability and Consistency} (as per CAP), but \textbf{else} (meaning even when the system has to run in absence of partitions) \textbf{one has to choose between Latency and Consistency}.
\section{Akka}
Set of \textbf{libraries for designing scalable, resilient systems that are concurrent and distributed}. Eases the task of \textbf{achieving reliable behavior, fault tolerance and high performance}.\\
To be successful, distributed systems must operate in environments where components crash without responding, messages get lost without trace and networks latency fluctuate.
Akka provides and relieves:
\begin{list}{}{}
	\item Multi-thread without using low-level constructs, relieving from focusing on memory visibility issues
	\item Transparent remote communication between systems, reliving writing and maintaining networking code
	\item Clustered, elastic and scalable on-demand architecture, enabling a reactive system
\end{list}
\end{document}