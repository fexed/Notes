...
\paragraph{}
\begin{list}{}{}
	\item Figure out config
	\item Define kernel: in OCL compile a string, in CUDA special keywords
	\item Direct command Kireagh queue: copy out, copy in, kernel exec
\end{list}
...
\subsection{FPGAs}
Devices with components that can be programmed to act as other components simply configuring them at the beginning, somehow. Basically: logical gates, small registers, small ALUs\ldots and a FPGA is an array where the single cell can act as if it were: a small (5-7bit input and 1bit output) boolean network, a 1 bit register or as router redirecting to a neighbor cell.
%TODO columns
...
\end{lstlisting}
\end{list}
FPGAs are very good at executing pipelined code. 
\paragraph{HLS} High Level Synthesis tools.
\end{document}