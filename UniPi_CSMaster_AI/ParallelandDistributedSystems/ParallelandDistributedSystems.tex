...
\end{center}
\paragraph{\texttt{ff\_node}} Computes some function and has two queues: a queue where to take items to be computed and a delivery queue.\\
...
In the \texttt{svc} method, being \texttt{void*} both in parameters and in return type, you can specify whichever type you need to flow through the tasks. You can pass a task as parameter, process it and cast to \texttt{void*} its result to return it. With \texttt{ff\_node\_t<t1, t2>} it's introduced the type checking ad template level.
...
\paragraph{\texttt{ff\_node\_t<tin, tout>}} Requires the type of the input and of the output. Requires to implement a methods\begin{list}{}{}
	...
	Has to \texttt{return(tout*)}, or a \texttt{return(EOS)} when it wants to end (passing to \texttt{svc\_end}, or a \texttt{return(GO\_ON)} that tells the framework that you finished the work but doesn't put anything in the output stream. Like a \texttt{skip} and "give me the next item".
\end{list}
...
\paragraph{Lifecycle of \texttt{ff\_node\_t}}\begin{enumerate}
	...
	\item \texttt{EOSnotify()} callback upon receiving \texttt{EOS}, to do something when receive \texttt{EOS} because upon receiving that we don't call \texttt{svc} but go to \texttt{EOSnotify()} and then \texttt{svc\_end}
\end{enumerate}
