...
\end{list}
The discount term is mathematically\ldots %TODO
...
Defines the behavior of an agent, MDP policises depends only on the current state (\textbf{Markovian}) and are \textbf{stationary} (time-independent): $A_t\sim \pi(\:\:\:|\:s)$ %TODO
...
\subparagraph{Theorem} For any Markov Decision process\begin{list}{}{}
	\item There exist an optimal policy %TODO
\end{list}
...
\paragraph{Value Iteration} Problem %TODO
Formally
...
So far: solve a known MDP (states, transition, rewards, actions). In a model-free context: no environment model neither knowledge of MDP transition/rewards %TODO
...
Uses the simplest idea: the value is the mean return across %TODO
...
\end{center}
Expected SARSA\\
We sample also future action $A'\simeq \pi$ (instead of leveraging policy %TODO
