...
\end{center}
\paragraph{Occlusions} Measure what happens to feature maps and object classification if we occlude part of the image. Slide a grey mask across %TODO
\paragraph{Dense CNN}
\begin{center}
...
\subparagraph{Contractiv Autoencoder} Penalize encoding function for input sensitivity.\begin{center}
	\includegraphics[scale=0.5]{78.png} %TODO rewrite
\end{center}
...
\end{center}
This is called a \textbf{Deep Belief Network}.\begin{center}
	\includegraphics[scale=0.5]{82.png}
\end{center}
It's \textbf{not recurrent}. Training requires attention because of the recurrent interactions from higher layers to the bottom.
$$P(h_j^1\:|\:x,h^2)=\sigma\left(\sum_jW_{ij}\ldots\right)$$ %TODO
$$P(x_i\:|\:h^1)=\sigma\left(\sum_jW_{ij}^1h_j^1\right)$$
To train this, first pretrain the first layers entails fitting this model
$$P(x\:|\:\Theta)$$ %TODO
Then pretrain the second layer changes $h^1$ prior by
$$P(h^1\:|\:W^2) = \sum_{h^2}P(h^1,h^2\:|\:W^2)$$
A trick, averaging the two models of $h^1$ can be approximated by taking half contribution from $W^1$ and half from $W^2$. Using full both would double count $x$ constributions as $h^2$ depends on $x$.
