...
Viterbi
\subsubsection{Forward-Backward Algorithm} Smoothing: how do we determine $P(S_t = i\:|\:\hat{y})$? We will compute $P(S_t=i, \hat{y})$, it's proportional (just divide by $P(\hat{y})$).\\
...
If I observe $S_T$ we block the path $y_{1:t}$
\begin{list}{}{}
	\item $\alpha = P(S_t=i,y_{1:t})$
	\item $\beta = P(y_{t+1:T}\:|\:S_t=i)$
\end{list}
\end{document}