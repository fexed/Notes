...
$$\Delta W=\alpha(v_\pi(S)-\hat{v}(S;W))$$ %TODO
...
Return $G_t$ is an unbiased %TODO
\paragraph{Value Function Approximation with TD(0)} The thing changes, because it's biased. %TODO
\paragraph{Value Function Approximation with TD($\lambda$)} Forward in the theoretical way, but we don't have that information so we use the backward
%TODO
...
\begin{list}{}{}
	...
	\item Policy improvement: $\epsilon$-greedy policy improvement
\end{list}
Approximate the action-value function $\hat{q}$ %TODO
So the update is %TODO
\paragraph{Linear Action-Value Function Approximation} Use table feature action-states %TODO
...
\paragraph{Deadly Triad} Loos guarantees of convergence, you can find counterexample that show you cannot converge %TODO
...
Repeat
\begin{enumerate}
	\item Sample state value from experience %TODO
	\item Apply stochastic gradient descent update %TODO
\end{enumerate}
...
\end{center}
\paragraph{Improvements on Original DQN} %TODO
\end{document}