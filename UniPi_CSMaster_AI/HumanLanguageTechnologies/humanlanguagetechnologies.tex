...
Treebank annotation schemes vary across languages, hard to coampare results across them. Also hard to use to build multilingual.
\begin{center}
	\includegraphics[scale=0.5]{39.png}
\end{center}
\paragraph{Goal} Facilitate consistent notation of similar constructions across languages. Support multilingual NLP and linguistic research. Build on common usage and existing de-facto standards. Complement and not replate language-specific schemes. \textbf{Community effort}.
%TODO
\paragraph{Morphological Annotation} 
\begin{list}{}{}
	\item \textbf{Lemma} represent the semantic content of a word
	\item \textbf{Part-of-speech} %TODO
\end{list}
...
\end{list}
\paragraph{Coordination} The first conjunct is the head of all following conjuncts. Attach coordinating conjunctions and punctuation to the immediately succeeding conjunct.\\
Except for the right headed constructions.
\paragraph{Enhanced Dependencies} Making some of the implicit relations between words more explicit, to facilitate relation extractions\begin{list}{}{}
	\item Propagation of conjuncts
	\item Conkjoined verb and phrases
	\item Subjcect from controller phrsase
	%TODO images
\end{list}
\paragraph{CoNLL-U Format} %TODO table
...
\end{center}
Keeping the content words as heads promotes parallelism acros languages. The main grammatical relations involving a passiv verb, a nominal subject and an oblique agent are the same.\\\\
Taxonomy of 42 universal grammatical realtions, broadly supported across languages in many panguage typologies. Language specific subtypes can be added.
\paragraph{Parsing}
...
\begin{list}{}{}
	...
	\item Stylistical and cultural differences 
\end{list}
...
\end{list}
\subsection{Language Similarities and Divergences} \paragraph{Typology} Systematic study of these similarities and divergences. %TODO
...
\begin{list}{}{}
	...
	\end{list}
\end{list}
In isolating languages a word can have one morpheme, while in polysynthetic languages single words can have many morphemes.\\
In agglunative languages morphemes have clean boundaries, in fusion languages a single affix can have many morphemes.\\\\
So there's a wide range of synthesis: from isolating (vietnamese) to englishm to russian to onedia (synthetic).\\
The other range is fusion: from agglunative (swahili) to russian and oneida (fusion).
...
\end{center}
\paragraph{Direct Translation} Proceed word-by-word in the text translating each, without %TODO
...
\paragraph{Transfer Model} We use a set of \textbf{transfer rules} that restructure the parse tree. But we need hard to obtain lexical transfer rules.
\paragraph{Lexical Transfer} Transfer-based systems also need lexical transfer rules, bilingual dictionaries. Can be a list or a word disambiguation system.
\subparagraph{Systram} Combination of direct and transfer. Performs morphological analysis and chunking. Then transfers with translation of idioms and word sense disambiguation, and apply a rich bilingual dictionary.\\
$N^2$ set of transfer rules, grammar and lexicon full of language-specific stuff, hard to build and maintain.
\paragraph{Interlingua} Instead of language-to-language rules, we abstract the meaning of the sentence and translate that.
%TODO pros and cons of all three.
...
\begin{list}{}{}
	\item %TODO
\end{list}
Faithfulness and fluency are formalized: best-translation of a source sentence S
$$\hat{T}=\arg\max_T \hbox{Fuency}(T)\hbox{Faithfulness}(T,S)$$
...
\begin{list}{}{} %TODO details
	\item \textbf{Language Model} 
	\item \textbf{Translation Model}
	\item \textbf{Decoding Algorithm}
\end{list}
\paragraph{Phrase Based Machine Translation}
\begin{list}{}{}
	\item Group words into phrases
	\item Translate each phrase
	\item Move the phrases around
\end{list}
$P(F\:|\:E)$ is modeled by translating phrases in $E$ to phrases in $F$. First segment $E$ into a sequence of phrases $\overline{e}_1,\ldots,\overline{e}_I$, then translate each $\overline{e}_i$ into $\overline{f}_i$ based on translation probability $\phi(\overline{f}_i\:|\:\overline{e}_i)$. Then, reorder translated phrases based on distortion probability $d(i)$ for the $i$the phrase %TODO
\paragraph{Tranlsation Probabilities} Assuming a phrase aligned parallel coprus is available or constructed that show matchin betewwn phrases in $E$ and $F$. Then compute (MLE) estimate of $\phi$ based on simple frequency counts $$ $$ %TODO
\paragraph{Distrortion probability} The probability that a phraase in position $X$ in the original sentence moves to position $Y$ in the tranlsation.\\
Distortion is the measure of distance between positions of corerspondign phrases in the 2 languages. Distortion of phrase $i$ as the distance between the start of the foreign phrase hgenereated bu $\overline{e}_i$ ($a_i$ and the end of the foreign phrase generated bu the previous phrase $\overline{e}_{i-1}$ ($b_{i-1}$). Typically we assume the probability of a distortion decreases exponentially with the distance of the movement $$d(i) = c\alpha^{|a_i-b_{i-1}|}$$ %TODO
Set $0<\alpha<1$ base on fit to phrase-aligned training data. Then set $c$ to normalize $d(i)$ so that it sums to 1.
\paragraph{Translation Probabilities} 
...
$$\phi(\overline{f},\overline{e})$$ %TODO
But we don't have phrase alignemtns. We have \textbf{word alignments}.
\begin{center}
	\includegraphics[scale=0.5]{43.png}
\end{center}
(Actually we have more restricted word alignments)\\
Word alignments are mapping between source and target words in parallel sentence. Restriction: each foreign word come from exactly one source word.
\begin{center}
...
\textbf{Spurios words} are words in the foreign sentence that does not align with any word in the source language. We assume one to many alignment: each word in $F$ aligns with 1 word in $E$, with some words in $F$ coming from NULL.
\paragraph{Computing Word Alignments} IBM-Model 1\\
%TODO
\paragraph{Training Alignment Probabilities}\begin{list}{}{}
	\item Get a parallel corpus
	\item Sentence alignemnt
	\item Use EM %TODO
\end{list}
\paragraph{Phrase-Based Translation Model}
\paragraph{Decoding}
