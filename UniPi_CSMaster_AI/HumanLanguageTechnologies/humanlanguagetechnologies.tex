...
\end{center}
%TODO racconto algoritmo fine 1a parte 02/05
\begin{enumerate}
...
\end{center}
\paragraph{Neural Models for Dialogue Response Generation} %TODO
...
\paragraph{Templates} Many requests can be answered with templates. Select the most relevant response from a collection of templates, determine the slots required to fill it and instantiate the template with those values. Slots filler %TODO
\paragraph{Retrieval-Based Chat} Basic idea: given an utterance, find the most similar in the database and return it. Similarity based on exact word match %TODO
...
Discourse-level Variable Autoencoder\\\\ %TODO
Diversity promoting objective to mitigate %TODO
...
\textbf{Personality infused dialogue} can address this. Train a generation system with controllable "knobs" based on personality traits, e.g. extraversion. Non-neural\\\\ %TODO
Another idea is \textbf{speaker embeddings}, that may learn general facts associated with the specific speaker.
\subsection{Google DialogFlow} DialogFlow is %TODO
\section{•}
\paragraph{Applications}
\begin{center}
	\includegraphics[scale=0.5]{132.png}
\end{center}
\paragraph{Attention Mechanism} Applied to NMY, effective in aspect based sentiment/topic classification and other tasks.
...
Possesses flexible "in-context" learning: GPT-3 demonstrates some level of fast adaption\\ %TODO
...
%skip
\subsection{Assessing} %TODO
\paragraph{Neural Network Effectiveness} %TODO image
\paragraph{Understanding Models by Breaking Them} Are our models robust to innocuous changes in their input? By robust, in this case we mean their outputs don't change.
\paragraph{Remark}
\begin{list}{}{}
	\item Neural Models are complex, fascinating objects that we don't currently understand, but we're making strides to understand them better
	\item A wide variety of analysis methods have been developed %TODO
\end{list}
\subsection{Inferring From Memory}
\paragraph{Current State} Transformer models have become quite popular and used everywhere. Neural methods are leading to a renaissance\ldots %TODO
...
\paragraph{Limits of Single Task Learning} Most models today are single task. Great performance improvements, %TODO
\paragraph{Multiple Tasks} How to express different task in the same framework it's still very hard.
\paragraph{Deep Linguistic Analysis} %TODO
%TODO
\paragraph{Back-Translation} Have two machine translation models going in opposite directions. No longer circular, models never see "bad translations" but only bad inputs.
\subsection{Huge Models} %TODO
