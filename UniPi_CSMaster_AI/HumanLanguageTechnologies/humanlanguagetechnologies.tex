...
$$C_{MAP}=\arg\max_{c_j\in C} P(x_1,\ldots,x_n\:|$$%TODO
\subsubsection{Naive Bayes Assumption}
\begin{list}{}{}
	\item $P(c_j)$ can be estimated from the frequency of classes in the training examples
	\item $P(x_1,\ldots,x_n\:|\:c_j)$ %TODO
\end{list}
The assumption is: features are independent of each other given the class
...
\paragraph{Smoothing to Avoid Overfitting} For example adding 1 to the counts so that it would never be zero (\textbf{Laplace}) %TODO formula
...
Multiplying lots of prob can result in floatingpoint underfglw. It's better to perform computations by summing logs of provbabilities since log(xy) = logx+logy\\
Class with highest final unnormalized log probability is still the moste probable %TODO formula
\paragraph{Generate} We can use naive bayes models to generate text, by using the probabilities of the words.
\paragraph{Naive Bayes and Language Modeling} Not the same thing, in naive we want to generalize and can use any sort of features. But if we only use %TODO
Each class corresponds to a unigram language model
