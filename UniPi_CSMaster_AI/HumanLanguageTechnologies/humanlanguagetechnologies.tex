...
\begin{list}{}{}
	...
	\item \textbf{Representation}: choose a set of features to represent data $x\mapsto \phi(x) \in \mathbb{R}^D$
...
\paragraph{Dense Representations} Project word vectors $v(t)$ into a low dimensional space $\mathbb{R}^k$ with $k<<|V|$ of continuous space word representations (a.k.a. \textbf{embeddings})
$$\hbox{Embed} : \mathbb{R}^{|V|}\rightarrow \mathbb{R}^k$$
...
\begin{multicols}{2}
\begin{list}{}{}
	\item Inputs are one-hot representation of the word, so $w\in \mathbb{R}^{|Voc|}$ is high dimensional as the Vocabulary may be very big
	\item The input word matrix is $V\in \mathbb{R}^{|Voc|\times d}$ for an embedding space of size $d$: an embedding $v\in \mathbb{R}^d$ is low dimensional.\\
	Row $t$ of $V$ is the input vector, representation for the \textbf{center} word $w_t$
	\item The output matrix $U\in \mathbb{R}^{d\times|Voc|}$ and each column $o$, the output vectors, is the representation for a \textbf{context} word $w_o$
...
\begin{list}{}{Procedure}
	\item Lookup the embedded word vector for the center word $v_c = V[c] \in \mathbb{R}^n$
...
\end{center}
\paragraph{Extrinsic Vector Evaluation} \textit{The proof of the pudding is in the eating}: \textbf{test on a task}, e.g. NER (Named Entity Recognition)
\subsubsection{Embeddings in Neural Networks} An embedding layer is often used as first layer in a neural network for processing text.\\
...
\begin{list}{}{}
	...
	"My \textit{cat} and your \textit{cat}", two different tokens of the same word
\end{list}
...
Data: $\{(x_i,y_i)\}$ for $i=1,\ldots,n$ with $x \in \mathbb{R}^d$ and $y \in \{-1,+1\}$. Question: find a \textbf{linear decision boundary} $wx + b$, an \textbf{hyperplane}, such that the classification rule associated with it has minimal probability of error.\\
...
\begin{list}{}{}
	...
	With gradients
	$$\mathlarger{\left(\begin{array}{c}
	\mathlarger{\frac{\partial L}{\partial f_\theta(x)}}\\
	\mathlarger{\frac{\partial L}{\partial f_\theta(x^c)}}
...
\end{center}
\paragraph{Context Free Grammars} $G = (N,\Sigma, \mathbb{R}, S)$
\begin{list}{}{}
	\item Set of \textbf{non-terminal symbols} $N$
	\item Set of \textbf{terminal symbols} $\Sigma$ disjoint from $N$
	\item \textbf{Set of rules}/productions $\mathbb{R}$ in the form $A\rightarrow\beta$ with $A$ non-terminal, $\beta$ string of symbols from the infinte set of strings $(\Sigma\cup N)^*$
	\item A designated \textbf{start symbol} $S \in N$
\end{list}
The language that $G$ generates, $L(G)$, is the set of every string that can be produced starting from $S$ with the rules in $\mathbb{R}$. Formally:
...
\subparagraph{Statistical Parsing} Three components \begin{list}{}{}
	...
	\item $\Phi$ maps a candidate to a feature vector $\in\mathbb{\mathbb{R}}^d$ and defines the representation of a candidate.\\
	...
	\item $W$ is a parameter vector $\in\mathbb{\mathbb{R}}^d$ and maps a candidate to a real-valued score.
\end{list}
...
\begin{list}{}{}
	...
	\item $H$ determines the semantic category of $D$ ($D$ specifies $H$)
...
\begin{list}{}{}
	...
	\item \textbf{Constraint Satisfaction}: edges that don't satisfy hard constraints are deleted.
\end{list}
...
\paragraph{Dependency Graph} Let $\mathbb{R} = \{r_1,\ldots,r_m\}$ the set of dependency types (the tags we'll put on the links).\\
A dependency graph for a sequence of words $W = w_1,\ldots,w_n$ is a labeled directed graph $D = (W,A)$ where\begin{list}{}{}
	...
	\item $A$ is a set of labeled arcs $(w_i, w_j, r)$ with $w_i,w_j\in W$ and $r \in \mathbb{R}$
...
\end{list}
\columnbreak
\paragraph{MT Not Yet Good Enough for...}
\begin{list}{}{}
...
\begin{list}{}{}
	...
	\item \textbf{Affixes}: prefixes, suffixes, infixes, circumfixes
	\vfill \null
\end{list}
\end{multicols}
...
We set $0<\alpha<1$ based on a fitting process to phrase-aligned training data, then set $c$ to normalize $d(i)$ so that it sums to 1.
...
\subparagraph{Attention Variants} We have some values $h_i\in \mathbb{R}^h$ and a query $s\in \mathbb{R}^h$. Attention involves:
\begin{list}{}{}
	\item Computing the attention scores $e\in \mathbb{R}^N$
	\item Taking softmax to get a attention distribution
	$$\alpha=\hbox{Softmax}(e) \in \mathbb{R}^NJ$$
	\item Using $\alpha$ to take the weighted sum of values
	$$\alpha = \sum_{i=1}^N\alpha_ih_i\in \mathbb{R}^h$$
...
The generic attention, as we have seen before: we have some \textbf{values} $v_1,\ldots,v_N\in \mathbb{R}^{d_1}$, some \textbf{keys} $k_1,\ldots,k_N\in \mathbb{R}^{d_1}$ and a \textbf{query} $s\in \mathbb{R}^{d_2}$\begin{list}{}{}
	\item Compute the \textbf{attention scores} $e\in \mathbb{R}^N$
	\item Taking softmax to get the \textbf{attention distribution} $\alpha=\hbox{Softmax}(e)\in \mathbb{R}^N$
	\item Using the attention distribution to take the weighted sum of values $$a = \sum_{i=1}^N\alpha_iv_i\in \mathbb{R}^{d_1}$$
	Thus obtaining the \textbf{attention output} or context vector $a$
\end{list}
There are variants, several ways to compute $e\in \mathbb{R}^{d_1}$:
\begin{list}{}{}
	\item Basic dot-product attention $e_i=s^Tk_i\in \mathbb{R}$\\
	This assumes $d_1=d_2$, and is the version used in NMT
	\item Multiplicative attention $e_i=s^TWk_i\in \mathbb{R}$\\
	Where $W\in \mathbb{R}^{d_1\times d_2}$ is a weight matrix
	\item Additive attention $e_i=w^T\tanh(W_1k_1+W_2s)\in \mathbb{R}$\\
	Where $W_1\in \mathbb{R}^{d_3\times d_1}$, $W_2\in \mathbb{R}^{d_3\times d_2}$ are weight matrices and $w\in \mathbb{R}^{d_3}$ is a weight vector.\\$d_3$ is the attention dimensionality, hyperparameter.
\end{list}
...
\end{list}
\subparagraph{Prediction Explanations} What in the input led to this output? For a single example, what parts of the input led to the observed prediction?\\ One way is via\textbf{saliency maps}: a score for each input word indicating its importance to the model's prediction.\\
...
\end{list}
Another method is \textbf{explanation by input reduction}: making changes to the document and measure how much it affects the result. What is the smallest part of the input I could keep and still get the same answer? Idea: run an \textbf{input saliency method}, iteratively removing the less important words.\\
Another way is \textbf{analyzing models by breaking them}: can we break models by making seemingly innocuous changes to the input?
\subsection{Probes}
...
\end{center}
We have some property $y$ (like POS). We have the model's word representations at fixed layer $h_1,\ldots,h_T$ where $h_i\in \mathbb{R}^d$ where the words are at indices $1,\ldots,T$. We also have a function family $F$ like the set of linear models, or 1-layer FFNN with fixed hyperparameters. We \textbf{freeze} the  parameters of the model, so its not finetuned, then we train our probe: a function $$\hat{y}\simeq f(h_i) \:\hbox{ with } f\in F$$
...
\paragraph{Contextual Representation of Language} These pretrained embeddings specify a function which maps elements $v$ in a (word) vocabulary $V$ to vectors $h\in \mathbb{R}^d$
$$f_{vocab} : V\rightarrow \mathbb{R}^d$$
$$v\mapsto h$$
...
\subparagraph{Finding a Parse Tree Encoding Distance Metric} Our potentially tree-encoding distances are parametrized by the linear transformations $B\in \mathbb{R}^{k\times n}$
...
\paragraph{SQuAD} Stanford Question Answering Dataset: 100k annotated $\langle$passage, question, answer$\rangle$ triples. Passages are selected from English Wikipedia, 100-150 words, and questions are crowd-sourced. Each answer is a short segment of text in the passage: this is a \textbf{limitation}, not all questions can be answered this way.\\
...
\begin{multicols}{2}
\vfill
$$\overrightarrow{c}_i = \hbox{LSTM}(\overrightarrow{c}_{i-1}, e(c_i))\in \mathbb{R}^H$$
$$\overleftarrow{c}_i = \hbox{LSTM}(\overleftarrow{c}_{i-1}, e(c_i))\in \mathbb{R}^H$$
$$c_i = [\overrightarrow{c_i};\overleftarrow{c_i}]\in \mathbb{R}^{2H}$$
\vfill
\columnbreak
\vfill
$$\overrightarrow{q}_i = \hbox{LSTM}(\overrightarrow{q}_{i-1}, e(q_i))\in \mathbb{R}^H$$
$$\overleftarrow{q}_i = \hbox{LSTM}(\overleftarrow{q}_{i-1}, e(q_i))\in \mathbb{R}^H$$
$$q_i = [\overrightarrow{q_i};\overleftarrow{q_i}]\in \mathbb{R}^{2H}$$
\vfill
\end{multicols}
\paragraph{Attention} 
\begin{center}
...
\begin{list}{}{}
	\item First, compute a similarity score for every pair $(c_i,q_i)$ with $w_{sim}\in \mathbb{R}^{6H}$
	$$S_{ij} = w^T_{sim}[c_i;q_i;c_i\odot q_i] \in \mathbb{R}$$
	\item Context-to-query attention (which question words are more relevant to $c_i$)
	$$\alpha_{ij} = \hbox{Softmax}_i(S_{ij})\in \mathbb{R}$$
	...
	$$\beta_i = \hbox{Softmax}_i\left(\max_{j=1}^M(S_{ij})\right) \in \mathbb{R}^N$$
	$$b = \sum_{i=1}^N\beta_ic_j\in \mathbb{R}^{2H}$$
\end{list}
The final output is
$$g_i=[c_i;q_i;c_i\odot a_i;c_i\odot b]\in \mathbb{R}^{8H}$$
...
\begin{list}{}{}
	...
	$$m_i=\hbox{BiLSTM}(g_i) \in \mathbb{R}^{2H}$$
	...
	$$m'_i = \hbox{BiLSTM}(m_i)\in \mathbb{R}^{2H}$$
...
\end{list}
The retriever is a standard TF-IDF information retrieval sparse model (a fixed module). The reader is a neural reading comprehension model trained on SQuAD and other distantly-supervised QA dataset.\\
...
\begin{list}{}{}
	\item $E\in \mathbb{R}^{|V|\times d}$ embedding matrix, $E(w) = e\in \mathbb{R}^d$
	\item $\{e_1^{(s_t)},\ldots,e_m^{(s_t)}\}$ vectors for the words in $s_t$
	\item $\{e_1^{(q)},\ldots,e_k^{(q)}\}$ vectors for the words in $q$
	\item $f^{(s/q)} = \{f_1,\ldots,f_m\}$ multiplicative mask with $f_i\in \mathbb{R}^d$
\end{list}
...
\end{center}
\subparagraph{Output Module} Scores memories $h_i$ using $q$, with an embedding matrix $E\in \mathbb{R}^{|V|\times d}$ to generate the output $\hat{y}$
$$p_i = \hbox{Softmax}(q^Th_i)$$
$$u = \sum_{j=1}^z p_ih_i$$
$$\hat{y} = \mathbb{R}\phi(q + Hu)$$
where $\hat{y}\in \mathbb{R}^{|V|}$ represents the model answer.
...
\subparagraph{Mention Linking} Use the question answering framework to compute $s_a(i,j)$. Transformer input triple: $\{$context, query, answers$\}=\{X,q,a\}$\begin{list}{}{}
	...
	\item $q(e_i)$ query: sentence containing $e_i$ replaced by $<$mention$>e_i<$/mention$>$
...
Intuition: there's an alignment between entities and words in the sentence, such that projections $W_tw_j$ and $W_ee_k$ are in the same vector space.
