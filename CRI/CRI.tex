...
\paragraph{Identificazione} %TODO
\paragraph{Autenticazione} %TODO
\paragraph{Firma Digitale} \begin{enumerate}
	\item Mittente non deve poter negare di aver inviato un messaggio $m$
	\item Destinatario deve essere in grado di autenticare il messaggio
	\item Destinatario non deve poter sostenere che $m'\neq m$ è il messaggio inviato dal mittente
\end{enumerate}
Tutto deve essere verificabile da una terza parte.
\paragraph{Relazioni tra le funzionalità}
\subsection{Funzioni hash} Funzione hash è una funzione $f:X\rightarrow Y$ tale che $n = |X| >> m = |Y|$\\
$\exists\:X_1,X_2,\ldots,X_m \subseteq X$ disgiunti tali che $X = X_1 \cup X_2 \cup \ldots \cup X_m$ e $\forall\:i,\forall\:x\in X_i$ ho $f(x) = y$\\
Comodo per gestire la rappresentazione compatta dei dati.\\\\
Una buona funzione hash assicura che:
\begin{list}{}{}
	\item I sottoinsiemi $X_1,\ldots,X_m$ hanno circa la stessa cardinalità %TODO
	\item %TODO
\end{list}
\subsection{Funzioni hash one-way} Se la funzione è usata in crittografia, deve soddisfare le seguenti proprietà:
\begin{enumerate}
	\item (comp facile: tempo polinomiale nella dimensione di x
	\item (comp difficile: esponenziale
	\item comp difficile trovare due elementi diversi con stessa immagine hash
\end{enumerate}
...
\begin{list}{}{}
	...
	\item \textbf{SHA}
\end{list}
Per proteggere da attacchi interni necessario hashare le password\\
Seme utile per non hashare password uguali di utenti diversi con stesso hash.\\
Protocollo1: messaggio m in chiaro. Posso sostituire m con c crittogramma? La verifica richiede la consocenza della chiave pubblica e restituisce m, quindi non avrebbe senso sostituire m con c.\\
...\\
