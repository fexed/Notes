...
$y^2 \equiv x^3 + ax + b$ mod $p$, e possiamo pensare di sostituire $m$ a $x$, il problema è che il risultato potrebbe non essere un residuo quadratico. $P(m^3+am+b$ sia un residuo quadratico$)$ circa 1/2\\
Residuo quadratico: ha radice nel campo.\\
Tecnica con metà di possibilità di successo non va bene, serve una tecnica migliore. Ci sono algoritmi randomizzati, non sono noti algoritmi deterministici polinomiali
...
\subparagraph{Svolgimento} \begin{enumerate}
	...
	Inoltre, un messaggio $m$ da 64 bit con $e=3$ non viene ridotto in modulo per un $n$ da 512 bit. Avrò sempre $c = m^3$.
