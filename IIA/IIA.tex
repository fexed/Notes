...
\paragraph{Concept Learning} Inferire una funzione booleana da esempi d'addestramento positivi e negativi. X spazio delle istanze, C: X $\rightarrow$ \{tt, ff\} oppure \{+, -\} oppure \{0, 1\}
\paragraph{Esempio} $<x, c(x)>$ in D (o training set)
\paragraph{Soddisfa} $h : X \rightarrow \{0, 1\}$ soddisfa $x$ se $h(x) = 1$
\paragraph{Consistenza} Un'ipotesi è consistente con l'intero D se è consistente se coincide con i valori target\\\\ %TODO
L'obiettivo nostro è ricostruire quell'\textit{unknown function}. Questo è un \textbf{problema inverso} (\textbf{ill posed}, o malposto), perché può violare l'esistenza, l'unicità o la stabilità della funzione.\\
Nel caso generale $|H| = 2^{\textsl{num istanze}} = 2^{2^n}$ per input binari, con $n =$ dimensione dell'input
\subsection{Regole congiuntive}
proposizioni fatte con l'and\\
posso mettere o non mettere quindi tutti i modi possibili sono 2$^n$, se includo anche il not diventano $3^n + 1$
%TODO descrizione ed esempio
\paragraph{Learning hypothesis}
...
\end{center}
L'idea è fare una ricerca in un oridne intelligente, smart. Parto dall'ip + specifica e mi sposto verso quella più generale, guardando istanze una ad uno che arrivano dalla tabella. Se l'ipotesi non soddisfa cerco di generalizzarla (rendere vera l'ipotesi) ma col passo minore possibile.
\subsection{Algoritmo Find-S}
\begin{enumerate}
	\item inizializziamo h con ipotesi più specifica
	\item per ciascuna istanza positiva, guardiamo attributi dell'ipotesi\\
	Se attributi sono soddisfatti dall'istanza nuova allora ok\\
	altrimenti bisogna cambiare attributi per generalizzare ipotesi e portarla ad 1. Facendo il minimo possibile
...
\begin{list}{}{}
	...
	\item Find-S produce l'ipotesi più specifica in H cosnistente con gli esempi d'allenamento positivi
	\item L'ipotesi di output h saràù anche consistente con gli esempi negativi, se il concetto obiettivo è contenuto in H, perché c $\geq$ h.
\end{list}
\subsection{Complaints about Find-S}
\paragraph{Version Spaces} è sottinisnieme ipotesi consistente con tutti gli esempi di addestramento
...
\paragraph{Rappresentare i version spaces}
\begin{list}{}{Definizioni}
	\item General boundary
	\item Specific boundary
	\item Teorema
\end{list}
\subsection{Algoritmo Candidate Elimination}
%TODO
grazie all'ordine parziale posso controllare di non avere cose più generali/specifiche e mantenere consistenza
\section{Bias Induttivo ed il suo ruolo}
Per ora abbiamo usato uno spazio delle ipotesi biased, vincolato. Abbiamo assunto che lo spazio delle ipotessi contenesse C, cioè che l'obiettivo C fosse espresso in and e non in or.\\
Supponiamo sistema di apprendimento unbiased, con H che esprime ogni concetto insegnabile: H è l'insieme di tutti i possibili subset di X: P(X).\\
Se $|X|$ = 96, $|P(X)| = 2^{96}$ cioè circa $10^{28}$.\\
Cosa succede nella generalizazione?
\paragraph{Definizione} Un unbiased learner è impossibilitato a generalizzare: ogni istanza non osservata, viene classificata positiva da precisamente delle ipotesi nel version space e negativa dall'altra metà. Per ogni h consistente con xi test esiste h' identica ad h eccetto h'(xi) diverso da h(xi), h in VS quindi h' in VS (identiche su D).\\Il bias non è solo assunto per efficienza ma anche necessario per la generalizzazione.
\paragraph{Bias Induttivo} Il bias induttivo di L è un set minimo di asserzioni B tale che per ogni concetto obiettivo C e dati di addestramento corrispondenti D$_C$,\\ %TODO
...
\begin{list}{}{\textbf{Ingredienti}}
	\item %TODO
\end{list}
\paragraph{Compito supervisionato} %TODO
\subsection{Problemi di regressione}
Processo di stima di una funz a val real su una base di un insieme finito di esempi rumorosi: coppie $(x, f(x) + \textsl{ rumore casuale})$. Un esempio è una tabella $x \mapsto$ valore che \begin{tabular}{c | c}
	$x$ & target\\
	\hline
	1 & 2.1\\
	2 & 3.9\\
	...
	5 & 9.8
\end{tabular}
Si può ipotizzare che $f(x) = 2x$, a mente: abbiamo applicato un algoritmo di machine learning assumendo la funzione lineare\ldots\\ %TODO
\paragraph{Definizione} Assumiamo un modello hw(x) espresso come w=w1x+w0\ldots %TODO
Trovare a partire dall'esempio un'ipotesi h che al meglio fa il fitting dei dati dai dati osservati x e y. Assumento che y sia collegata linearmente ad un'altra variabile x o altre variablili y= w1x + w0 + rumore, dove le w sono libere. costruiamo il modello cercando di trovare i valori di w per predirre e stimare y per altri valori di x non osservati.
%TODO esem
\paragraph{Costruire il modello} Apprendimento tramite LMS. Dato insieme esempi training, trovare hw(x) nella forma (w1+ w0) che minimizza l'errore medio. Least mean square: $$\textsl{loss}(h_w) = E(w) = \sum_{p=1}^l (y_p - h_w(x_p))^2$$ con valori al quadrato per non avere negativi \ldots\\
Perché LMS per il migliore $h$? Minimizzo la distanza dalla media (linea blu) , approccio standard mper approssimare la soluzione di sistemi sovra-determinati: n eq più delle incognite.
\paragraph{Come risolvere} Punto di minimo è stazionario dove il gradiente è nullo. Bisogna cercare fra quelli a gradiente nullo $$\frac{\partial E(w)}{\partial w_i} = 0$$ quindi per la regressione lineare (a due parametri) $$\frac{\partial E(w)}{\partial w_0} = 0 \:\: \frac{\partial E(w)}{\partial w_1} = 0$$
...
Per $\frac{\partial E(w)}{\partial w_1} = \frac{\partial y}{\partial w_1} - (\frac{\partial w_1 x}{\partial w_1} + \frac{\partial w_0}{\partial w_1}) = 0 - (x + 0)$
\paragraph{Riassumendo} Dato un set l insieme set di trainingi come coppie xpyp, trovare i valori di w costruendo hw(x) espressoa come w1x + w0. Trovata si può usare per esprimere nuovi valori. La linea diretta è meno interessante, preferiamo il gradiente per l'approccio della ricerca locale.\\
...
$\Delta w$ è il "passo" che si fa in direzione del gradiente e $\eta$ costante di apprendimento.
def error correction (delta rule) che cambia la w proporzionalmente all'errore
\paragraph{Per pattern $l$}
\begin{center}
	\includegraphics[scale=0.55]{patternl.png}
\end{center}
aggiornaimo w \ldots\\ %TODO
\paragraph{Input multidimensionale} caso standard: da 2 a migliaia di input/variabili: pattern di input è un vettore
\begin{list}{}{Esempi}
	\item %TODO
\end{list}
\paragraph{DATA notation} $X$ è una matrice $l \times n$, $l$ righe, $n$ colonne (variabili)\\
$p = 1..l$, $j = 1..n$\\\\
$
...
\end{array}$\\
%TODO each row...
Notazione: x come vettore con indice pattern nella prima posizione %TODO
\begin{center}
	\includegraphics[scale=0.55]{riassuntone.png}
\end{center}
\subsection{Gradient descent Algorithm}
Semplice algoritmo sul quale si basano molti di quelli attuali
\begin{enumerate}
	\item Inizio con un vettore di pesi $w_{initial}$ inizializzato con valori piccoli, fisso $\eta$ (0 $< \eta <$ 1)
	\item Si calcola $\Delta w = -$ "gradiente di $E(w)$ = $- \frac{\partial E(w)}{\partial w}$ (per ogni $w_i$)
	\item Si fa il passo, calcolando $w_{new} = w_{old} + \eta\cdot\Delta w$ (per ogni $w_i$)\\
	Con $\eta$ dimensione del passo, o \textbf{coefficiente di apprendimento}
	\item Ripeto da 2 finche converge o $E(w)$ è \textit{sufficientemente piccolo}
\end{enumerate}
\begin{list}{}{}
	\item $\Delta w / l$ LMN (least mean squares)
	\item Batch versions ($\Delta w$ dopo ogni epoca di dati $l$) vedi prima
	\item %TODO
	\item $eta$: tasso di apprendimento, trade-off su velocità/stabilità. Può essere gradualmente ridotto a 0: garantisce la convergenza, evita oscillazione intorno al minore)
\end{list}
\paragraph{Esempi di curva di apprendimento} Curva di appr: elettrocardiogramma del modello. Se prendo modello, applico i dati e l'algoritmo 
\begin{center}
	\includegraphics[scale=0.5]{curveappre.png}\\
...
\begin{list}{}{}
	...
	\item Facile da interpretare: praticati in medicina, biologia, chimica\ldots
	\item Dati rumorosi ammessi
	\item Statistici contenti (proprietà interessanti)
	\item Fenomeni lineari: un sogno per la scienza
	\item \textbf{Baseline per l'apprendimento}: è un problema lineare?
	\item Usato/incluso in modelli più complessi
\end{list}
Ma hanno molte limitazioni evidenti: soprattutto su funzioni non lineari.\\
Possiamo introdurre, al posto delle $x$ originali delle trasformazioni non lineari: $x^2$, $x^3$,\ldots: $w_0 + w_1 x + w_w x^2 + w_3 x^3$\ldots che corrisponde ad una \textbf{regressione polinomiale} = $$y(x, w) = \sum_{j=0}^M w_j x^j$$ si può sempre risolvere con Least Square solution. Questo perché ad esempio posso chiamare $x^2 = z_2$ e niente cambia nella tecnica risolutiva.\\
\textbf{Linear Basis Expansion} (LBE)
$$h_w(x) = \sum_{k=0}^K w_k \phi_k (x)$$
Aumento gli input e ciascuna trasformazione di $x$ tramite la funzione $\phi_k : R^n \rightarrow R$\\
$\phi$ può essere una qualsiasi trasformazione: la norma, il quadrato, logaritmo\ldots\\
Sto espandendo l'insieme delle variabili considerate, quindi $K > n$, lineare nei parametri e in $\phi$ ma non in $x$: possiamo usare i soliti algoritmi lineari di prima.
\paragraph{Esempi}
\textbf{LBE} è anche detto \textbf{approccio a dizionario}: le $\phi$ sono le parole a disposizione per comporre le "frasi": più espressivo, modella relazioni non lineari. Svantaggio: se introduco troppe trasformazioni possiamo avere un modello \textit{troppo} complesso rispetto alle necessità (\textbf{overfitting}).
\paragraph{Trade-Off} sulla complessità del modello:
\begin{list}{}{}
	\item Modello troppo semplice non fa buon fitting, underfitting, soluzione biased
	\item Modello troppo complesso: sensibile a perturbazione dei dati, molta varianza ed overfitting
\end{list}
metodi di \textbf{regolarizzazione del modello} per \textbf{trovare bialnciamento cotnrollo complessità modello}. Penalizzando funzioni troppo complesse (riducendo valore dei pesi) mantenendo flessibilità spazio ipotesi.\\
Rasoio di Occam: la spiegazione più semplice è spesso la più corretta. \textit{Preferire le ipotesi più semplici che corrispondono ai dati}.\\
Questo è un concetto fondamentale in ML, che troveremo di nuovo quando "razionalizzaremo" alla fine. Ora introduciamo un approccio per i modelli lineari: regolarizazione con ridge regression
\section{Ridge Regression}
\paragraph{Rikhonov Regularization} Bassa varianza, meno overfitting. Modello smoothed.\\
Introduce vincol iche controllino somma val nostri parametri w favorendo modelli sparsi, abbassando valore w. Se alcuni li portiamo bassi in particolare a 0, anche se abbiamo introdotto molte funzioni alcune spariscono.
$$\textsl{Loss}(h_w) = \sum_p (y_p + h_w(x_p))^2 + \lambda ||w||^2 $$ con nuova aggiunta alla nostra funzione di loss: $\lambda ||w||^2$, con $\lambda$ \textbf{coefficiente di regolarizzazione} (parametro costante) e $||w||^2 = \sum w_i^2$\\
Quando minimizzo l'errore cerco anche di tenere i $w$ bassi, perché la Loss deve riuscire ad abbassare entrambi i termini. Devo anche fare altre considerazioni\ldots\\
Efftto: decatimento del peso (in pratica aggiungo $2\lambda w$ al gradiente
$$w_{new} = w_{old} + \eta\cdot\Delta w - 2\lambda w_{old}$$\\
Se $\lambda$ è alto allora modello fortemente normalizzato, se $\lambda$ basso allora dò molta importanza ai dati quindi rischio overfitting.
\paragraph{Limitazione delle funzioni base expansion} %TODO p. 63
$\phi$ sono fissate \textit{prima} di osservare i dati.
\chapter{Classificazione}
\paragraph{Classificazione di modelli lineari} Riusiamo il modello lineare Attribuiamo categoria ad es 0/1, -1/+1\ldots\\
Usiamo l'iperpiano ($wx$) asssumendo val pos e neg.\\
Decidiamo se $x$ appartiene a zona positiva o negativa. Vogliamo trovare $w$ tramite apprendimento
\paragraph{Decision boundary} $w^T x = w_1x_1 + w_2x_2 + w_0 = 0$ punto d'intersezione fra i due piani.\\
Metto scalino: tutte le volte che ero in zona poisitiva attribuisco valori uscita ad 1, dall'altra parte 0/-1
hx = 1 se wx + w0 magguguale 0, o 0 altrimenti\\
Oppure h(x) = sign(wx + w0)
Iperpiano separatore: insieme dei punti per cui il nostro modello è 0. Quando maggug 0 possiamo dire 1 altrimenti 0. LTU: linear threadhold unit.
\paragraph{Def} Dato un set di $l$ esempi d'apprendimento, trovare $w$ da \textbf{minimizzare} la somma di quadrati $$E(w) = \sum_{p=1}^l (y_p - x_p^Tw)^2 = ||y - Xw||^2$$
Non usiamo $h(x)$ perché in classificazione ci starei mettendo $h(x) = \textsl{sign}(wx)$ che \textbf{non è differenziabile}.\\\\
Abbiamo ancora l'algoritmo iterative gradient descent
$$\Delta w_i = - \frac{\partial E(w)}{\partial w_i} = \sum_{p=1}^l (y_p - x_p^Tw)\cdot x_{pi}$$
\end{document}
