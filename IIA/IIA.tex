...
Usano una KB come \textbf{insieme di clausole}\\
\{A, B\} \{$\neg$B, C, D\} \{$\neg$A, F\}\\
La forma a clausole è la \textbf{forma normale congiuntiva} (\textbf{CNF}): una congiunzione di disgiunzioni letterali\\
\{A $\vee$ B\} $\wedge$ \{$\neg$B $\vee$ C $\vee$ D\} $\wedge$ \{$\neg$A $\vee$ F\}\\
\textbf{Non è restrittiva}: è sempre possibile ottenerla con trasformazioni che preservano l'equivalenza logica.
%https://elearning.di.unipi.it/pluginfile.php/30946/mod_resource/content/1/KRProp.pdf, p. 35
...
%TODO
\section{Metodi locali per SAT}
Gli stati sono assegnamenti completi.\\
L'obiettivo è un adssegnamento che soddisfa tuttel e lcausole (un modello). Si parte da assegnamento casuale\ldots\\ %TODO
Algoritmi.\\
Molti minimi locali per sfuggire ai quali serve introdurre perturbazioni casuali. Ad esempio, hill climbing con riavvio casuale, simulated annealing\ldots molta sperimentazione per trovare miglior compromesso tra grado di avidità e casualità.\\
WALK-SAT è uno degli algoritmi più semplici ed efficaci.\\
\subsection{Algoritmo WalkSAT}
WalkSAT ad ongi poasso sceglie a caso clausola non soddisfatta. Poi sceglie simbolo da modificare (flip) scegliendo con probabilità p (di solito 0,5) tra una delle due possibilità seguenti: sceglie un simbolo a caso da flippare (passo casuale) o sceglie quello che rende più clausole soddisfatte (passo di ottimizzazione).\\
Si arrende dopo un numero predefinito di flip.\\
\begin{center}
	Algoritmo
\end{center}
\paragraph{Un esempio}
\paragraph{Analisi} Se maxflips è inf e insieme clausole è soddisfacibile prima o poi termina, ma non può essere usato per verificare insoddisfacibilità (è incompleto). Problema decidibile ma algoritmo incompleto.\\
Va bene per cercare modello, sapendo che c'è. Ma se è insoddisfacibile, non termina.\\
Lo si usa perché è efficiente in tempo e spazio.
\paragraph{Problemi SAT difficili} se problema ha molte soluzioni (sotto-vincolato) è più probabile che walksat ne trovi una in tempi brevi. es 16 sol su 32, assegnamento 50\% prob di essere giusto: 2 passi in media\\
Più grande è il rapporto più vincolato è il problerma. Regine è facile perché vincolato
\paragraph{Probabilità di soddisfacibilità in funzione di $\frac{m}{n}$} $m$ clausole e $n$ simboli, media di 100 problemi generati a caso.
\begin{center}
	\includegraphics[scale=0.7]{sat_grafico.png}
\end{center}
\paragraph{Confronto dpll w walksat}
\begin{center}
	\includegraphics[scale=0.7]{dpllwalksat_confronto.png}
\end{center}
\section{Inferenza come Deduzione}
Un altro modo per decidere se KB $\vDash$ A è usare un meccanismo di \textbf{dimostrazione} (\textbf{deduzione}). Se scrive KB $\vdash$ A (A è deducibile da KB) e la deduzione avviene specificando delle regole di inferenza: dovrebbero derivare solo le formule che sono conseguenza logica e \textbf{tutte} le formule che sono conseguenza logica.\\
...
\subparagraph{Direzione} Nella dimostrazione di teoremi conviene procedere all'indietro. Con un applicazione in avanti delle regole di inferenza non controllata. Da A, B, A anmd B, A and (A and B)\ldots\\
All'indietro dimostro A, poi B, poi A and B.
\subparagraph{Strategia} Anche assumendo insieme di regole di inferenza completo se l'algoritmo non è completo potrei non trovare soluzione
\paragraph{Regola di risoluzione per PROP} Meno regole sono meglio è. Unica regola di inferenza è la regola di risoluzione.\\
Con due clausole, una P e l'altra che contiene notP, possiamo considerare or degli altri elementi togliendo P e notP.
\subparagraph{In generale}
%TODO
$$\frac{\{l_1, l_2,\ldots }{robe da sistemare}$$
