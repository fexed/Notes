...
%TODO https://elearning.di.unipi.it/pluginfile.php/31082/mod_resource/content/3/KR-Rules2020.pdf, p. 21
...
Albero SLD per il goal \texttt{antenato(lia, mark)}\\ %TODO
...
Apprendimento è una sfida strategica per fornire intelligenza ai sistemi.\\
Problema complesso ,campo che cresce continuamento e riguarda sapetti teorici e applicativi: apprendimento automatico / machine learning\\
Com bina scopo di creare macchine che possono apprendere con sistemi statistici potenti utilizzabili in vari anbiti con approfondimento rigoroso nella scienza computazionale.
\paragraph{Macchine che imparano da sé} Perché? Lusso o necessità?
\begin{list}{}{}
	\item credsente bisogno di analisi di dati empirici
	\item ruolo centrale e metodologico per il cambio nel paradigma della scienza: data-driven
	\item difficile fornire intelligenza attraverso la programmazione (vedi Turing). Ha molto più senso lasciare che le macchine apprendano da sole, non si può insegnare loro tutto.
	\item apprendimento è l'unica via\ldots
\end{list}
Dati + HPC + ML moderno $\rightarrow$ strada verso la nuova era delle IA
\begin{list}{}{}
	\item Costruire sistemi intelligenti adattivi
	\item Data analysis per costruire sistemi predittivi potenti
	\item Modelli come strumenti per risolvere problemi complessi ed interdisciplinari
\end{list}
Apprendimento automatico di un isstema dell'esperienza (serie di esempi) per affrontare un task computazionale. \textbf{Partire da esempi}
\paragraph{Esempio} \textbf{Classificare lo spam}. Collezionare 100 mail di spam, 100 non di spam e uso come esempi, fornisco al sistema che apprende per poter essere ccapace di classificare spam e non in futuro.
...
\paragraph{Quando và applicato?} Strumento molto potente, ma va capito quando applicarlo: ha dei limiti. Utile apprendimento predittivo quando non esiste o è scarsa o è difficile da formalizzare la teoria attorno ad un problema. O anche quando i dati sono incerti, rumorosi o incompleti.\\
Componenti personalizzati (ambienti dinamici): quei casi in cui il comportamento è specifico per persona (per me è spam, per un'altra persona no). Non si può completamente definire il comportamento a priori.\\
...
\paragraph{Perché?} %TODO
\begin{center}
	\includegraphics[scale=0.55]{ml1.png}
\end{center}
L'apprendimento vogliamo vederlo come approssimazione di una funzione non nota attraverso degli esempi.\\
Facciamo un esempio intuitivo: riconoscimento caratteri scritti a mano. Ingresso è collezione di immagini (array/matrici di valori) e costruire un modello che ne assegni il valore rappresentato.
\begin{center}
	\includegraphics[scale=0.55]{ml2.png}
\end{center}
Ha tutte le caratteristiche elencate prima: difficile da formalizzare, presenza di rumore nei dati e di dati ambigui. Ma facile collezionare esempi etichettati. $\Rightarrow$ esempio di applicazione ottima del ML.\\
...
\paragraph{Tasks} Supervised Learning\\
Abbiamo delle coppie date da qualcuno $<$\textit{input} $x$, \textit{output} $d$ $>$ per una funzione non nota $f$
\textbf{Target Value} Valore desiderato $d$ o $y$ o $y$\ldots dato dal supervisore\\
\textbf{Vogliamo trovare una buona approssimazione di $f$}: un'ipotesi $h$ che può essere usata per prevedere un dato nuovo $x'$
\paragraph{Target} un'\textbf{etichetta} numerica/categorica\\
\textbf{Classificazione} $f(x)$ ritorna la corretta classe di $x$ ipotizzata\\
\textbf{Regressione} approssima una funzione obiettivo a valori reali\\
...
\end{center}
\paragraph{Approccio non-Supervisionato} Non c'è il supervisore: serie di esempi non etichettati, ad esempio per trovare raggruppamenti naturali dei dati.
\paragraph{Modello} Ha lo scopo di catturare e descrivere relazione fra dati. Definisce la classe delle funzioni che il sistema di apprendimento può implementare (\textbf{spazio delle ipotesi})
\paragraph{esempi di apprendimento} esempi della forma x, f(x), x vettori di valori, f(x)=t è la funzione target
\paragraph{Ipotesi} proponiamo noi, la $h$ espresisone in un qualche linguaggio che si crede simile ad $f$ ignota
\paragraph{Spazio delle ipotsi} insieme di tutte le ipotesi che possono essere soluzione
\paragraph{Linguaggi per l'H} Logica del prim'ordine, equazioni numeriche, ma anche probabilità
...
Apprendimento è quindi ricerca di una \textbf{buona} funzione. Cosa significa buona: \textbf{quando ha una buona capacità di generalizzazione} o quindi avere un basso errore di generalizzazione. \textbf{Alta accuratezza su dati nuovi}.\\
...
\textbf{Performance in ML = accuratezza predittiva} stimata dell'errore calcolata sul \textbf{test set}.
\paragraph{Teoria} Sotto quali condizioni matematiche un modello è in grado di generalizzare? Statistical Learning Theory.\\
\begin{center}
	\includegraphics[scale=0.55]{mlstrutt.png}
\end{center}
