...
Generalizzo il problema. \textbf{Apprendimento supervisionato} (classificazione e regressione): supervisore che etichetta gli esempi. Spazio di input: $x \rightarrow_f$ cateogrie o valori reali. Costruisco una funzione attraverso gli esempi.
\pagebreak
...
Più alto è l'information gain, più efficace è nella classificare i dati.
...
$h$ overfitta i dati di apprendimento se c'è un'ipotesi alternativa $h' \in H$ tale che $E_D(h) < E_D(h'$) e $E_x(h') < E_x(h)$, cioè $h'$ si comporta peggio sui TR e meglio sui dati non osservati.\\
...
\begin{list}{}{}
	...
	\item minimum description lenght, usa misura complessità e mi fermo quando cresce troppo oltre una certa misura (similitudine con Tikhonov)
\end{list}
Ogni nodo è candidato per il pruning. Pruning consiste rimuove un sottoalbero radicato in un certo nodo, che diventa foglia assegnata\ldots\\
Nodi rimossi solo se risulta albero non peggiore sul valid set.\\
Nodi pruned iterativamente: ad ogni iterazione il nodo la cui rimozione aumenta maggiormente l'accuratezza è tagliato.
\section{Validazione}
...
\end{list}
bisogna cercare $h$ in $H$ tale che minimizzi $R$. Abbiamo però solo il data set finito TR = $($x$_i, d_i)$ ($i = 1..l$)\\ Per cercare $h$ bisogna minimizzare l'errore in fitting cioè il \textbf{rischio empirico} $$R_{emp} = \frac{1}{l}\sum_{i = 1}^l \left(d_i - h(\text{x}_i)\right) ^2$$
...
Con $\epsilon$ detta \textbf{VC-Confidence}, funzione direttamente proporzionale a VC (VC-dim) ed inversamente proporzionale a $l$ numero dei dati e $\delta$ (detta confidenza, guida la probabilità che valga il bound: $\delta$ piccolo 0.01 $\Rightarrow$ probabilità 0.99).
...
Porta a nuovi modelli (SVM) e altri modelli che direttamente considerino il controllo della complessità nella costruzione.\\
...
\section{K-Means} Il più semplice e più usato algoritmo che implementa la squared error. Popolare perché facile da implementare e efficiente in generale, ma ha diverse limitazioni che vedremo.\\L'\textbf{algoritmo} base è il seguente:
\begin{enumerate}
	...
	\item Assegniamo ogni pattern al cluster più vicino, cioè al cluster il cui centro è più vicino al pattern (il winner)
	\item[$\uparrow$]
