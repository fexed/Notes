...
\begin{list}{}{}
	\item $\exists x$ Padre(x, G
\end{list}
\textbf{Grounding} o \textbf{proposizionalizzazione}\\
Creao tante istanze delle formule quantif univers quanti sono oggetti menzionati\\
Eliminare quantificatori esistenziali skolemizzando. KB diventa proposizionale e applicare algoritmi visti, ma problemi: costanti in numeri finito, ma se ci sono funzioni il numero di istanze da creare è infinito john, padre(john), padre(padre(john))\ldots
\subsection{Teorema di Herbrand}
Se KB soddisfa A allora c'è dim che coinvolge solo sottins finito della KB proposizionalizzata.\\
Si può procedere incrementalmente
\begin{list}{}{}
	\item creo istanze co costanti
	\item creo quelel con un solo livello di annidamento Padre(john)
	\item creo quelle con due livelli padre(padre(j))
\end{list}
Se KB non soddsifa A, il processo non termina. \textbf{Semidecidibile}.
\paragraph{Forma a clausole}
costanti, funz, pred come definiti, escludiamo formule atomiche del tipo (t1 = t2)\\
Una clausola è insieme di letterali (formula atomica eventualmente negata) che rappresenta la loro \textbf{disgiunzione}.
\paragraph{Trasformazione in forma a clausole}
th: per ogni formula chiusa alfa del FOL è possibile torvare in maniera effettiva un insieme di clasuole FC(alfa) soddisfacibile sse alfa lo era (insoddisfacibile sse alfa lo era)
\paragraph{Esempio trasformazione}
\textbf{skolemizzazione non preserva l'equivalenza}
\paragraph{Unificazione} operazione con la quale si determina se due espressioni possono essere rese identiche mediante una sostituzione di temrini a variabili.\\
riosultato è la sostituzione che rende le due espressioni dientifce, detta \textbf{unificatore}, o \textbf{FAIL} se le espressioni non sono unificabili
\paragraph{Sostituzione} insieme finito di associazioni tra var e termini in cui ogni var compare una sola volta sulla sx. Es \{x1/A, x2/f(x3), x3/B\} significa che A va sostituita a x1, f(x3) sost a x2\ldots\\
A sx var a dx costanti e var, con la restriz che la var sulla sx non può comparire anche sulla dx della stessa "coppia".
\paragraph{Applicazione} Sia $\sigma$ sost e A espressione, Asigma istanza generala dalla sost (delle var con corrisp espress)\\
In AIMA \texttt{SUBST($\sigma$, A)}. Var sostituite simultaneamente e eseguo un solo passo di sostituzione
\paragraph{Espressioni unificabili} se esiuste sostituz che le rende identiche (unificatore).  In genere l'unificatore $\tau$ non è unico. Vorremmo l'unificatore più generale di tutti (MGU). \textbf{Teorema}: l'\textbf{unificatore più generale è unico} a meno dei nomi delle variabili (l'ordine non conta)
\paragraph{Algoritmo di unificazione} input p,q espressioni, restituisce MGU $\Theta$ se esiste, \texttt{unify(p,q) = theta} tale che \texttt{subst(theta, p) = subst(theta, q)}, altrimenti \textbf{fail}.\\
Esplora in parallelo le due espressioni e costruisce l'unificatore strada facendo. Fallisce non appena trova espressioni non unificabili. Una causa del fallimento sono sost del tipo x=f(x), questo controllo si chiama \textbf{occur-check}.
\begin{center}
	Algoritmo
\end{center}
\paragraph{Esempi}
...
\begin{list}{}{}
	...
	Sussunte (implicate)\\ %TODO esempio
...
\begin{list}{}{}
	...
	Ma non è completa\\ %TODO esempio
...
Possno essere riscritte come fatti e regole\\ %TODO esempio
Una \textbf{KB a regole} %TODO esempio
...
\paragraph{Uso delel regole in avanti e indietro} %TODO scrivere
\begin{list}{}{}
...
programmi logici sono kb costituiti di clausole horn definite, espressi come fatti e regole con sintassi alternativa %TODO esempio
\\
