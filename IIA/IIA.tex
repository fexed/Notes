...
\begin{list}{}{}
	...
	\item Viene fornito un input $x$ (dimensione $n$)
	\item Per rispondere troviamo il training example più vicino $x_i$\\
	Trovare l'indice $i$ con la minima distanza da $x$ $i(x) =$ arg min $d(x,x_j)$\\
	La distanza è la classica distanza euclidea, per esempio, cioè: $d(x, x_j) = ||x - x_k||$
	\item Come output $y_i$
\end{list}
Può creare superficie di separazione molto flessibile
\end{document}
