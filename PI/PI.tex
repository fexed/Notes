...
noi tutti crediamo di sapere ma ignoriamo completamente\\
\paragraph{Le persone come fanno le cose?} Finchè le cose vanno bene sembra siano semplici, e ci sembra chiaro come l'utente fa le cose e anche a lui sembra chiaro come ha ottenuto risultato. Quando cose vanno male l'interpretazione è opposa, il modo in cui le persone fanno cose è complesso perché i fenomini mentali in atto quando si fanno cose sono complessi. Quando cose vanno bene non ci se ne rende conto, quando ci sono problemi ce ne rendiamo conto in una serie di fasi.\\
Si da per scontato persone eseguono azioni, ma prima di tutto persone scelgono azioni da fare. Si dà per scontato che scelga l'azione scelta dal progettista.\\\\
immagino utente e mondo fisico/device. esische golfo esecuzione e golho della valutazione. Analogia con tanto più è profondo il golfo tanta più è la strada da fare per percorrerlo.
golfo esecuz utenti cercare capire cosa fare, una volta risolto golfo esecuz eseguono azione. macchina dà feedback e parte feedback nell'interpretare il feddback (cioè risultato mia azione su macchina)\\\\
Il golfo della valutazione è ritenuto dai designer semplice da superare. nel golfo valutaz. c'è qntà effort mentale che utente usa per comprendere interfaccia e capire se azione che aveva deciso di fare è stata eseguita come da lui pianificato.\\
se modello mentale differisce da concettuale il golfo valutazione sarà grande.\\\\
quali elementi design contribuiscono alla mitigazione del golfo dell'esecuzione: significanti, costraint, mapping e modello concettuale.\\
utente ha obiettivo, pianifica azione e la esegue. Dispositivo risponde attraverso interface display, utente interpreta e poi valuta.\\
elementi contribuiscono a mitigazione golfo valutazione: feedback e modello concettuale.
\paragraph{•} frustrazione persone quando non capiscono interfaccia nasce quando valutazione fallisce
\paragraph{Fare ed Interpretare} in entrambi i casi bisogna garantire understanding (cioè discoverability e visibility, se non capisco quando ottengo punti ma vedo aumentarli non capisco perché ottengo punti perché non è facilmente discoverable)\\
\subsection{Sette stati dell'azione} sette stati attraverso i quali avvengono i due golfi\\
Esecuzione\\
1- utente deve identificare il suo obiettivo\\
2- utente deve pianificare ciò che deve fare\\
3- pianificazione convertita in task specifici
4- che vengono eseguiti (clicco pulsante, scrivo i campi...)\\
Valutazione\\
5- ho visto cosa è accaduto (feedback)\\
6- lo devo interpretare (non è detto che io abbia strumento corretto da interpretare o che feedback sia sufficiente da essere interpretato)\\
7- comparare risultato interpretazione con il proprio obiettivo, e non devono differire.\\\\
goal-plan-specify-perform-perceive-interpreting-evaluate\\\\
fatica 2-4 = fatica golfo esecuzione, fatica 5-7 = fatica golfo valutazione.\\
non è detto che tutti i comportamenti passano da tutte le fasi. abitudine porta ad abilitare altre parti cervello dove non si ha più bisogno di specify (spacchettare azioni) o dove salto pianificazione (perché al goal associo già la seq più veloce che conosco già). Altra situazione è feedback che non si può comparare al goal e che quindi abilita nuovo processo (esempio compro ma non registrato, inserisci i dati feedback inizia nuovo processo)\\
trucco è sviluppare skill per utilizzare questo pardigma di analisi interazioni per capire se le nostre interfaccia performano bene o no.
