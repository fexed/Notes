...
identiicato problema (task analysis) come creo elementi individuati?\\
primo passo da fare dopo task analysis è identificare le personas. è l'archetipo di uno dei nostri utenti. \textit{la casalinga di voghera}, \textit{anziano}, \textit{infante}. Identifichiamo personas subito dopo task analysis perchè durante essa ci si rende conto che non tutti si comportano nello stesso modo. Si rende necessario creare archetipi.\\
macro task analysis con delle note da cui genero personas.\\
nel produrre personas si innesca processo dove mentre scrivo personas affino la task analysis.\\
scrivere personas mette a fuoco la userbase. colma la distanza tra azienda e cliente, perché ci si prova a impersoanre in uno dei clienti scrivendone la figura. è l'unico modo per provare davvero a capire l'utente. \textbf{mettersi nei panni dell'utente}.\\
tipicamente personas caratterizzata da un \textbf{goal}.\\
come arrivo a definire personas oltre task analysis anche tramite focus group.\\\\
quante personas scrivere? uno dei principi fondamentali al mondo è il \textbf{principio di pareto}: 80\% effetti generato dal 20\% delle cause.\\
\section{Requirements}
proprietà, serivzio, funzione\ldots perculiarità che il mio sistema deve avere al fine di assolvere al need utente. Persona -> need -> requirement.\\
Possono essere funzioni ma anche constraint obv. utente minorenne ha requirement di un costraint su certi siti che fa lockout\\\\
scrivere requirements è complesso, oneroso e cozza col metodo agile. Sempre pronti al cambiamento ma se faccio requirement-driven development si rischia di essere poco flessibili al cambiament, per questo requirement stanno perdendo di moda.\\
divisi in funzionali (feature) e non funzionali (qualità, divisi anche in categorie di utent)
\section{User Stories}
prossimo passo è scrittura user stories. tipicamente c'è una o più user story per personas, non ci possono essere più personas per story (significa troppe persona). se story non copre tutte sfaccettature singola personas allora dovrei dividerla, forse è personas troppo larga.\\
user story è requirement espresso dalla prospettiva dell'utente.\\
esempio confronto tra user story e requirement\\
aiuta a migliorare le personas\\\\
per scriverle $"as a [role] i want [feature] because [reason]"$ con elementi tra quadre presi dalla personas. Role non è personas, potrebbe esserlo.\\\\
chi scrive le user stories: chiunque può scrivere le user stories.
\section{Scenarios}
Scenario è evoluzione delle user stories. Io con user stories critallizzo piccoli pezzo di testo che associate a personas fanno da nota di quello che queste personas fanno con il nostro software. lo scanrio estende questo. inizia a catturare anche \textit{come} utente si comporta su sito app software. Come non nell'accezione dei task spacchettati ma in generale. siccome user stories dicono i need dell'utente, nello scenario estendo la user story. Ovviamente permangono goal personas ma si estendono, li si describono in maniera più ampia.\\
Scenario divisi in singoli use case.\\
Scenarios a cosa servono? Con le user story noi programmatori possiamo lavorare, dicono poco, le scrivo tutte ma dicono cosa sviluppare. Scenarios utili per interloquire con gli stakeholder (colui che è interessato all'output del progetto: cliente, capo, project manager\ldots). Questo perchè esterni al projecct le user stories non sono comprensibili, mentre gli scenarios danno immediatamente l'immagine dell'obiettivo.\\
Scenarios fondamentali per allinearsi con richieste del cliente. Senza loro si rischia di finire a presentare progetto finito senza che cliente sia soddisfatto.\\
Una volta approvati scenario: non ho fissato requirements e software che ottiene scenarios approvati garantisco soddisfazione cliente.\\
\subsection{Cosa tenere in considerazione quando si scrivono scenarios}
\begin{enumerate}
\item Chi è l'utente? tecnico
\item qual è la motivazione che lo ha spinto ad arrivare da noi? perché sistema che fa report automatico
\item qual è il suo obiettivo? fare report automatico
\end{enumerate}
In alcuni casi scenario potrebbero includere aspetti di \textbf{come} fare cose, ma ciò appartiene al mondo degli \textbf{use case}. Preferibile scenarios privi di tale aspetto.
\subsection{Come scrivere uno scenario}
Prima di tutto mapping. devo aver già personas e relative user stories. Scrivo scenarios che racchiudono una o più user stories relative a personas. Tipicamente per ogni personas almeno uno scenario.\\
user stories per debuggare personas\\
scenarios per debuggare le user stories\\
scnarios mettono in discussioni user stories che mettono in discussione personas\ldots\\
\textbf{Focalizzazrsi sul key task}: task fondamentale di quello scenario. Es. utente che compra il biglietto.\\
Scnarios contestualizzano il goal dell'utente (goal comprare biglietto, contestualizzo sul mobile\ldots)\\
Sviluppare mindset dedicato a risolvere questo problema. importante in scrittura scenario riuscire a esprimere qual è il goal dell'utente e com'è influenzato dal contesto dello specifico utente, della prior knowledge e del background (personas).\\
scenario portano user stories al next level integrando con l'interazione che avviene tra utente e prodotto o servizio della story.\\
\texttt{link sulla slide How To Write a Scenario}\\
scenario relativi all'\textbf{esperienza} della personas.\\
link tre metodi per scrivere scenario\begin{enumerate}
\item Goal o Task -Oriented
\item Elaborated Scenarios
\item Full scale task scenarios, da evitare. Diventa del tutto uguale agli use cases.
\end{enumerate}
Conviene partire da goal/task-oriented e ci si aggiungono elementi di contesto.\\
Parto da estensione di un paio di user story. Percorso di debug a partire dall personas, trovo elementi di contesto o particolarità da aggiungere. Senza troppo dettaglio.
\section{Use Cases}
Evoluzione dello scnario, dove dato scenario ho la narrativa vista dal punto di vista dell'utente. Cioè che cosa utente fa nella nostra piattaforma per svolgere quello scneario.\\
...
Fondamentale importanza perché vanno direttamente al team di sviluppo. Con use case ben fatti lo sviluppo è molto pià facile e semplice implementazione.\\
cosa c'è in use case: utente, cosa vuol fare, qual è goal finale, singoli step, feedback che l'intefaccia da durante step\\
...
\subsection{Elementi dello use case}
\ldots\\
main task (basic flow)\\
alternative paths (alternative flow)\\
\subsection{Come scriverlo}
\begin{enumerate}
\item identificare utenti personas
\item sceglierne uno
\item identificare i goal
\item discernere task principali (basi flow) dagli altri. legge 80-20: focalizzarsi sul main path perché 80\% utenti necessita del 20\% di features.\\Non gestisco carte smagnetizzate ma non posso permettermi di non dare soldi a nessuni (basi path, risolvere sempre)
\item con tutti i casi studio, si integrano eliminano ecc. ciò che si può
\item \ldots slides\ldots
\end{enumerate}
\pagebreak
