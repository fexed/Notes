...
We get $$h_a(k) = \sum_{i=0}^{r-1} a_i\cdot k_i\hbox{ mod }n$$ with $a\neq 0$ and $n$ prime number. So $h_a$ is a class of functions, $\# h_a = u-1$\\\\
Let's prove that it's universal, so the property $(\# h_a\:|\:h_a(x)=h_a(y))\leq\frac{u-1}{m}$ for any given $x\neq y$. We have $x,y$ of $\log_2 u$ bits divided into blocks. Since $x\neq y$, at least one of the blocks are different. Let's assume $x_0\neq y_0$ for simplicity of notation. For any pair of blocks it's analogous.\\
$\sum_{i=0}$ %todo 29/11 from 1:03:00
\end{document}