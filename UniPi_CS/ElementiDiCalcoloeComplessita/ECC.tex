...
\left\{
\begin{array}{l l}
	k & \text{se }M_i(n)\downarrow \text{in meno di }k\text{ passi}\\
	0 & \text{altrimenti}
\end{array}
\right.
\end{math}\\
...
Inoltre, per dimostrare che le funzioni non calcolabili esistono e sono molte di più, si usa un procedimento simile a quello di Cantor. Si crea una tabella dove enumeriamo sulle righe tutte le funzioni calcolabili, e in colonna il risultato per ogni valore:
\begin{center}

\begin{tabular}{c || c | c | c | c | c | c}
	 & $f_i(0)$ & $f_i(1)$ & $f_i(2)$ & $f_i(3)$ & $f_i(4)$ & \ldots \\
	 \hline
	 \hline
	 $f_0$ & 0 & 1 & 0 & 0 & 1 & \ldots \\
	 \hline
...
\end{tabular}
\end{center}
Ora possiamo prendere la diagonale $\delta$, che nell'esempio avrà valori $\delta = 0 1 0 0 \ldots$. La complementiamo, $\overline{\delta} = 1 0 1 1 \ldots$ ottenendo una nuova funzione che è diversa da tutte le altre in almeno un punto (poiché prendiamo da ogni funzione il risultato diagonale e lo complementiamo, analogamente possiamo aggiungere 1, l'importante è che lo si renda diverso).\\
Abbiamo ottenuto una funzione diversa da tutte le altre, che non appare nella lista delle funzioni calcolabili: di conseguenza, $\overline{\delta}$ non è calcolabile.
\paragraph{Notazione} Quindi indichiamo con $\phi_i$ la funzione (in generale, parziale) calcolata dall'algoritmo M$_i$ e $i$ \textbf{indice della macchina}. Come detto prima, può darsi che per $i \neq j$ sia $\phi_i = \phi_j$ ma sicuramente M$_i \neq$ M$_j$.
...
$\exists$ un predicato $T(i, x, y)$ e una funzione $U(y)$ ricorsive primitive calcolabili totali tali che $\forall$ funzione calcolabile $i$,\\$\phi_i(x)$ = $U(\mu$y.[$T(i, x, y)$]$)$
...
\paragraph{Dimostrazione} $\phi_i(x) = U(\mu y.T(i, x, y))$ per il terzo teorema.\\
$U(\mu y.T(i, x, y))$ è un algoritmo, poiché definito per composizione e $\mu$-ricorsione a partire da funzioni primitive ricorsive. Quindi avrà un indice per Church-Turing che indicheremo con $z$, e due parametri $i$ e $x$\\
Diciamo $\phi_z(i, x) = U(\mu y.T(i, x, y))$ (senza $y$ come argomento perché quantificata, non libera ma legata, una sorta di \textit{variabile di lavoro}). Applico la transitività dell'uguaglianza e ho finito, $\phi_i(x) = \phi_z(i, x)$.\\
...
\section{Ricorsivamente Enumerabile}
...
\pagebreak
\subsection{Teorema di equivalenza fra caratterizzazioni}
$I$ è ricorsivamente enumerabile $\Leftrightarrow$ $I = \emptyset \vee I = \textsl{imm}(f)$, con $f$ calcolabile totale.
\begin{multicols}{2}
\subparagraph{Dim} 
Il primo caso, $I = \emptyset$, è banale: la funzione caratteristica dà sempre 0, ricorsiva, e quindi risulta\\$I =$ Imm$(\lambda x.\textsl{indefinita})$\\\\
Nel caso $I \neq \emptyset$, $I = \textsl{dom}(\phi_i)$ costruisco la $f$ calcolata dalla $\phi_i$ con la tabella a fianco, dove:
\begin{list}{}{}
	\item[$m$] L'indice di riga tiene traccia del numero di passi di $\phi_i(n)$
	\item[$n$] L'indice di colonna tiene traccia dell'argomento
\end{list}
\columnbreak
\begin{center}
...
\end{center}
\end{multicols}
Più precisamente: in $(m,n)$ si eseguono $m$ passi del calcolo di $\phi_i(n)$. Si esegue: 1 passo in 0, 2 passi in 0, 1 passo in 1, 3 passi in 0\ldots finché non si trova qualche valore di $m$ e dell'argomento $n$ per cui il calcolo si arresta (cioè $\phi_i(\langle m,n\rangle)\downarrow$ in $m$ passi $\Rightarrow \langle m,n\rangle\in I$). , chiamo $\langle m,n\rangle$ il valore della codifica della coppia $(m,n)$ (Ad esempio, $\langle 2,1\rangle = 4$)\\\\
A questo punto ho un valore $\overline{n}\in I$.\\
Si riesegue un altro procedimento a coda di colomba eseguendo $\phi_i(n)$ per $m$ passi: se si arresta, allora si pone $f(\langle m,n\rangle) = n$ (ovviamente $n\in\textsl{dom}(\phi_i) = I$), altrimenti si pone $f(\langle m, n\rangle) = \overline{n}$ (e per quanto detto prima $\overline{n}\in I$).\\
Si itera incrementando la codifica di $\langle m,n\rangle$, generando così tutti gli elementi di $I$.
...
Dato che $\textsl{logspace}\subseteq\mathscr{P} \Rightarrow \leq_\mathscr{P}$ classificano $\mathscr{D}$ ed $\mathscr{E}$. Lo si dimostra così, tramite le proprietà della classificazione di una famiglia di riduzioni:
\begin{enumerate}
	\item $\textsl{id} \in \textsl{logspace}$, banalmente perché non scrive niente sul nastro di lavoro.
