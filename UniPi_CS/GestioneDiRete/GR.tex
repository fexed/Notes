...
\begin{list}{}{Altri esempi:}
	\item \texttt{1.3.1.3.6} $\rightarrow$ \texttt{3}
...
\paragraph{Object Identity} \texttt{nome OBJECT-IDENTITY, STATUS, DESCRIPTION, REFERENCE ::= <ObjectIdentifier>}\\
Definisce e registra un valore su un OID, permette l'allocazione di qualsiasi nodo sull'albero di registrazione.\\
\texttt{STATUS} definisce se il nodo allocato è obsoleto, corrente o deprecato, mentre il \texttt{REFERENCE} opzionale è usato per riferire informazioni aggiuntive (tipo link HTML)
\paragraph{Object Type} \texttt{nome OBJECT-TYPE, SYNTAX, UNITS, STATUS}\ldots\texttt{::= <ObjectIdentifier>}\\
Macro per la definizione di object types e tabelle concettuali. \texttt{INDEX} e \texttt{AUGMENTS} sono permesse solo nella definizione di tabelle. \texttt{SYNTAX} definisce il tipo, ad esempio \texttt{Integer32} o \texttt{SEQUENCE OF Integer32}.
\paragraph{Notification Type} \texttt{nome NOTIFICATION-TYPE, OBJECTS, STATUS}\ldots\texttt{::= <ObjectIdentifier>}\\
Macro per la registrazione di un evento, con \texttt{OBJECT} posso anche inviare oggetti interessati insieme alla trap.\\
\texttt{linkDown} per segnalare se ha perso il link, \texttt{linkUp} per collegamento preso.
\begin{center}
...
\begin{center}
	\includegraphics[scale=0.45]{mibii.png}\\
...
\begin{center}
	\includegraphics[scale=0.5]{snmppkts.png}
\end{center}
...
Le misure attive spesso sono end to end, mentre le passive sono link to link. Non c'è un approccio migliore o peggiore, ma è meglio combinare i due approcci. Si esegue analisi passiva aggiungendo controlli attivi per determinate metriche. Anche perché ad esempio il monitoraggio passivo su reti switchate può dare problemi, o ad esempio iniettare traffico su un link satellitare spesso può farlo solo il provider.
...
Header contiene:\begin{list}{}{}
	...
	\item \textbf{engine\_id} numero dello slot dove è stato rilevato il flusso all'interno dell'engine, per distinguere il mittente del flusso
\end{list}
Flusso contiene:
\begin{multicols}{2}
 \begin{list}{}{}
...
\paragraph{Integrated Monitoring} sFlow sugli switch, caratterizza ciò che legge SNMP con RMON su switch compatibili L2/L3. NetFlow sui router per la parte geografica.\\$\rightarrow$ soluzioni di analisi del traffico: sorveglianza continua sulla rete\ldots
\begin{multicols}{2}
\begin{list}{}{}
...
\end{center}
Il \textbf{client} è nella \textbf{centrale dell'operatore}/access point sul soffitto del Fibonacci, il server contiene l'anagrafica ed è un server dell'operatore/dell'UniPi con i dati degli utenti. Client comunica con il server che mantiene info sugli utenti e risponderà si/no per accettare l'autenticazione in base ai dati forniti dal client.\\
Dopo accesso c'è l'accounting request start: da ora navighi e calcolo informazioni per fare "il conto". A fine si manda richiesta di stop e in base a quello si calcola  costo. Quindi c'è comunicazione costante col server RADIUS (\textbf{interim update}, dove comunica aggiornamenti sul traffico dell'utente), in modo da poter interrompere comunicazione se superate eventuali soglie (e non scoprire il superamento solo al logout).\\
