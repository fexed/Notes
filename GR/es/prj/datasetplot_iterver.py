...
if (alpha != -1 and beta != -1 and gamma != -1): #Holt-Winters
	...
	for i in range(1000):
		...
		bestlst.append(res[0])
		iterazionitotali += iterazioni
		print("\r\033[F\033[K" + str(i) + ". " + "{0:0=3d}".format(iterazionitotali) + " iterazioni. Last SSE = " + "{:.6f}".format(bestlst[-1][1]))

	bestlst.sort(key = lambda x : x[1])
	alpha = bestlst[0][0][0]
	beta = bestlst[0][0][1]
	gamma = bestlst[0][0][2]
	...
	print("Dopo " + str(iterazionitotali) + " iterazioni ho trovato:\n\talpha = " + stralpha + "\n\tbeta = " + strbeta + "\n\tgamma = " + strgamma)
