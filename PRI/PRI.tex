...
\section{Changing Conventions}
Viene da pensare che innovaz debba essere segno discontin con passato (disruptive innovations), ma far digerire questo agli utenti non innovativi è un problema. Ogni volta che si viola convenzione (culturale, legale, tecnologica, anche frutto di pessime abitudini...) si chiede all'utente di fare un nuovo passaggio di apprendimento: genera \textbf{friction}, attrito. Se devo cambiare abitudini ci poenso due volte, perché fa fatica.\\
Quindi il processo di cambiare convenzioni dovrebbe essere graduale.\\
Cercare di trovare \textbf{modo per rimanere consistenti}. Qnt innovazi ad ogni passo tale da portare utente avanti ma non essere percepita come diversa. Il modello mentale che lui si è costruito deve rimanere tale, solo un po' allargato. La volta dopo si allarga un po' di più ecc.\\
La cosa peggiore che si può fare è quella di innovare lasciando il vecchio sistema presente per un po'. I sistemi misti confondono perché così l'utente crea nuovo mod mentale che è un mix dei due. Perché l'utente non si chiede il perchè del nuovo modello, se gli diamo poss scegliere lei va sul sicuro entra in crisi.\\
...
\subsection{Constraints} non si può progettare LA ma si progetta PER UX. Non si può vincolare più di tanto utente a fare specifiche azioni. Si possono usare constraints perché utente quando non ha visto cosa prima si fa modello mentale mischiando conoscenza pregressa. Si può limitare utente nella libertà d'azione. Il vincolo più famoso è la pila/batteria, al contrario la molla fa fatica (affordance, significante (+ e -), disegno ecc.) eppure sistema costraint che evita inversione batterie. PErché vincoli fisici? perché non è detto che vincolo culturale sia omnipresente. Vincolo fisico è ultima spiaggia.\\
A volte è possibile usare anche solo vincoli.\\\\
I costraint che tipicamente si trovano in interfaccie si dividono in 4 macrocategorie\\
physical: un pezzo entra solo in un verso, tappo biro sta solo in un verso da ambedue le parti ma nell'altro verso non entra\\
cultural: si guida a dx non a sx, maglietta sul torace\\ 
semantic: del significato, limite di velocità (semantica della segnaletica stradale, con il numero)\\
logical: \\\\
se non si ha costraint e mapping si genera frustrazione. costraint e mapping delle volte quasi si sfumano e confondono. interruttori messi sulla piantina è mapping così forte che è quasi un costraint logico: non ti puoi permettere di sbagliare\\
\subsection{Activity-Centered Controls}
quando si progettano controlli tipicamente i controlli sono associati alle funzioni.\\
invece di avere il pulsante abbassa telo, pulsante alimenta proiettore, pulsante accendi proiettore... io faccio un pulsante presentazioni con slide che fa tutto.\\
Le activity-centered controls devono essere disegnate sugli utenti, non sullo sviluppatore. Elettricista programmera pulsante presentazioni con la sua idea di presentazione, e probabilmente sarà super completa ma talmente specifica che non andrà bene a nessun utente. Invece bisogna fare user activity-centered control: chiedersi chi è utente ecc. e fare quella scena di presentazione che abilita entrambi che per uno va bene così com'è mentre per un altro potrebbe essere incompleta e premere un altro bottone.\\
altro errore comune è apparente activity-centered control ma in realtà creare device-centered control.\\
\subsection{Forcing functions}
è una forma di costraint fisico, ad esempio \textbf{interlock}: obbligare succesione di azioni all'utente. (premere pedale per usare la pressa idraulica). interlock vincola ad esegurie serie di passi per raggiungere certo stato/obiettivo, o anche per guidare il learning\\
\textbf{lock-in} invece blocco tutto, freezo, metto in pausa attuale situazione fino a che utente non ha fatto determinata cosa. Non puoi uscire da editing documento se consciamente non mi hai detto di salvarlo o buttarlo. A volte lock-in funziona così bene che diventa shortcut (chiudo e uso finestrella lock-in invece di andare col mouse a premere tasto).\\
\textbf{lockout} è l'opposto, ti chiude fuori finché non fai qualcosa (v.m. 18, assolve a richiesta legale posso dichiarare il falso ma così il reato l'ho fatto io e non il sito).\\
lockout azione prima di iniziare, lockin azione prima di concludere, interlock azione dopo serie di passi.\\
