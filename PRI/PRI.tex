...
Altro elemento fondamentale design interfacce è feedback inteso come da interfaccia verso utente. Es. Feedback della luce del pulsante premuto nell'ascensore, o "ok pagamento eseguito" a termine procedura pagamento.\\
Il feedback deve essere immediato. Sistema sensoriale parte integrante del sistema cognitivo. Uso i miei sensi per guidare i miei ragionamenti. Se faccio interfaccia che non abilita i miei sensi a capire cosa sto facendo entro in crisi, faccio più fatica a usare o no riesco proprio. Caso esempio: pagina web che non mostra se sta caricando la procedura richiesta.\\
Uno dei problemi principali del feedback è il tempo. Se faccio azione devo avere feedback entro certo tempo. se è superato, il mio cervello non è più in grado di associare evento all'azione. 2 pessimi risultati: no feedback e confusione utente.\\
\textbf{Feedback entro 100ms da azione, altrimenti (in generale) non sarà efficace}. Meglio un buon feedback che un bel feedback.\\
Inoltre feedback deve essere informativo: non significa tanta informazione ma deve assolvere all'obiettivo. Se premo pulsante non ho bisogno di fare granché, posso far diventare grigio il bottone (non serve, ok pulsante premuto ecc. è superfluo). Se converto pulsante in rosso non è più informativo: non capisco se ricevuto richiesta o errore (mapping naturale colore-significato, rosso è problema. Stesso discorso col verde, perché azione a buon fine ma magari non è ancora stato calcolato).\\
Feedbacki informativo nell'accezione dell'azione con cui è associato.\\
Meglio no feedback rispetto a feedback errato.\\
Non essere troppo pedanti. Se feedback eccessivo, l'interfaccia diventa pesante.\\
Altro problema feedback non allineato con contesto utilizzo del dispositivo. Es. non usare stesso beep cinture per segnalare la riserva.\\
Non limitarsi alla tecnologia disponibile (ho solo quel buzzer, non posso fare altrimenti: no, non sono obbligato a fare bepp faccio lampeggiare spia)
\subsubsection{Conceptual Model}
Modello concettuale è descrizione estremam semplificata del funz del sistema.\\
esempio classico i file e le cartelle. Come racconto a qualcuno com'è organizzata memoria archiviaz di un computer? Uso modello concettuale noto, cioè fogli di carta con contenuti, raccolti in raccoglitori e raccoglitori in schedari.\\
...
Pensato modello concett e disegnato. Implementata interfaccia per fare si che modello concett venga veicolato. Quando persona s'interfaccia con sistema sviluppa un \textbf{modello mentale}. Se modello mentale e concettuale sono allineati, persona è in grado di usare il sistema. Più è grande la differenza tra modello mentalee  concettuale più la persona farà fatica.\\
...
La gente apprende i modelli concettuali direttamente dal device per tentativi, ma il dramma è quando lo apprendi per passaparola. telefono senza filo, da persona a persona cambia l'iterpretazione. Per ogni disallineamento tra concett e mentale la cosa cambia. Per questo necessità concett pressoché univoco col mentale. Più piccolo è il delta, meno rischio di creare comportamenti assurdi col passaparole. Less is more, se feature difficile da veicolare non metterla.\\
...
utente non può chiedere a svilupp come funziona, ma deve scoprire funzionalità da solo. la teoria del'imm di sis dice che utente sviluppa uno user model (modello utente) grazie a oggetto e elementi a contorno (manuale istr, pubblicità, passaparola...). Progettare modello concettuale è solo parte dell'immagine di sistema. Aiuto utente a sviluppare modello mentale vicino concettuale se lo doto di altre informazioni: blog del software ad esempio.\\
