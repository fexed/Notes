...
Più precisamente: in $(m,n)$ si eseguono $m$ passi del calcolo di $\phi_i(n)$. Si esegue finché non si trova qualche valore di $m$ e dell'argomento, sia $\overline{n}$, per cui il calcolo si arresta (cioè $\phi_i(\overline{n})\downarrow$ in $m$ passi $\Rightarrow \overline{n}\in I$)\\\\
A questo punto, chiamo $\langle m,n\rangle$ il valore della codifica della coppia $(m,n)$. Si riesegue un altro procedimento a coda di colomba eseguendo $\phi_i(n)$ per $m$ passi: se si arresta, allora si pone $f(\langle m,n\rangle) = n$ (ovviamente $n\in\textsl{dom}(\phi_i) = I$), altrimenti si pone $f(\langle m, n\rangle) = \overline{n}$ (e per quanto detto prima $\overline{n}\in I$).\\
