...
\end{list}
\paragraph{Esercizio 10} Si dimostri che $\textsc{tot} = \{x\:|\:dom(\phi_x) = N\}\leq_{rec}\textsc{inf} = \{x\:|\:dom(\phi_x)$ è infinito$\}$
...
\begin{list}{}{}
	...
	Questo perché se $x\in\textsc{tot}$ allora $\phi_x$ ha dominio uguale a N, quindi presa una $y$, $\phi_x(z)$ con $z < y$ convergerà sempre e questo vale $\forall\:y\in N$, che è infinito, rendendo il dominio di $\psi(x,y) = \phi_{f(x)}(y)$ infinito.
	\item $x\not\in\textsc{tot} \Rightarrow\exists\:\overline{y}\:\forall\:y>\overline{y}$ ho $\phi_{f(x)}(y)$ indefinita $\Rightarrow f(x) \not\in \textsc{inf}$\\
	Perché se $\phi_x$ non ha dominio pari a N, allora posso trovare una $z < y$ per cui $\phi_x(z)\uparrow$. Avendo una situazione di indefinito, il dominio di $\phi_{f(x)}$ non può essere infinito.
\end{list}
...
\begin{list}{}{}
	...
	\item $x\not\in\overline{K} \Leftrightarrow x \in K \Rightarrow \phi_{f(x)} = \lambda y.1 \Rightarrow dom(\phi_{f(x)}) = N \Rightarrow f(x) \not\in \textsc{fin}$
\end{list}
Nota: è immediato vedere che $\emptyset\neq\textsc{fin}\neq N$, da cui, per il lemma usato  per dimostrare il teorema di Rice, segue che non è ricorsivo.
\pagebreak
...
\subparagraph{Svolgimento} Banalmente $\emptyset\neq I \neq N$ e inoltre $I$ è un insieme di indici che rappresentano funzioni, pertanto $I$ non è ricorsivo.\\
...
\end{array}\right.$
\begin{list}{}{}
	\item $x\in K\Rightarrow \phi_{g(x)}(y) = 1 \Rightarrow dom(\phi_{g(x)}) = N \Rightarrow g(x) \not\in I$
