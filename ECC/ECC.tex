...
Sia $\mathscr{F}$ un insieme di funzioni calcolabili.\\
L'insieme $A = \{x | \phi_x \in \mathscr{F}\}$ è \textbf{ricorsivo} $\Leftrightarrow$ $\mathscr{F} = \emptyset$ oppure $\mathscr{F}$ \textbf{contiene tutte le funzioni calcolabili}.
\paragraph{Dim} Corollario del lemma precedente.\\
$A = \emptyset$ allora ok
$A = N$ allora $\mathscr{F}$ rappresenta tutte le funzioni calcolabili quindi $A$ ricorsivo, altrimenti $K$ si riduce ad $A$ o $\overline{A}$ e si riduce alla dimostrazione di prima.\\
Si noti che $A$ è un insieme di indici, mentre $\mathscr{F}$ è una \textbf{classe di funzioni}: $A$ è \textbf{sintassi}, mentre $\mathscr{F}$ è \textbf{semantica}.
...
\paragraph{TIME} La \textbf{classe di complessità} TIME$(f) = \{$problemi $I\:|\: \exists\: M$ che decide $I$ in tempo deterministico $f\}$\\\\
...
Grazie a questi risultati definisco $\mathscr{P}$ come $$\mathscr{P} = \bigcup_{k \geq 1} \textsl{TIME}(|n|^k) $$
%TODO lezione 23/04
\section{}
Macchine di turing a k nastri eventualmente di tipo I/O\\
per ogni x macchina (m, x \ldots) $\rightarrow^t$ M situazione di alt\\
$t \leq f(x+1)$ somma i a k-1 somma nastri di lavoro approssimata per eccesso da f(x)\\
TIME(I | esiste M)\\
SPACE(I | esiste M)\\
Riesce perché aggiungere nastri e avere I/O, o anche allargare la parola su cui lavorano le machcine quindi compattare simboli sono tutte operazioni algoritmicamente effettive: si possono fare con algoritmi. Dà bel vantaggio.\\
Però sorge problema: dobbiamo decidere un insieme (risolvere un problema) che sappiamo essere decidibile e una funzione che sappiamo essere calcolabile, ma il problema per cui non riusciamo a individuare struttura matematica e proprietà tramite le quali risolverlo \textit{facilmente}.\\
Per esempio macchina che decideva stringa palindroma abbiamo sfruttato una proprietà delle stringa: leggibile da dx a sx e da sx a dx. In questo caso è banale, ma ci sono problemi per cui non conosciamo la struttura e ciò rende la vita difficile.
...
\begin{list}{}{}
	\item $\Delta \subseteq (Q\times\Sigma)\times(Q\times\{\textsl{si, no}\}\times\Sigma\times\{Q, L, -\})$
	\item $\gamma = (q, u\sigma v) \rightarrow (q', u'\sigma'v')$
	\item Computazione $M(x) \rightarrow\gamma_1\rightarrow\ldots\rightarrow\gamma_n$ cioè $M(x) \rightarrow^n\gamma_n$
\end{list}
\textbf{Basta che ci sia un cammino che porti alla soluzione} perché il problema sia risolto.
...
\paragraph{Come decide un problema una macchina non deterministica} 
...
%TODO albero p. 155
Numerando archi di decisione identifico una computazione con la successione di numeri. Enumerare data certa sigma ho tante quintuple in delta, le enumero e le ordino in qualche modo. Prendo la $n_1$ scelta dallo stato iniziale, la $n_2$ scelta dallo stato risultante\ldots\\
...
\end{list}
NTIME(f) = \{I $|$ $\exists$ N dcide $I$ in tempo non deterministico $f$\}\\
$\mathscr{NP} = \bigcup_{k\geq 1}\textsl{NTIME}(n^k)$
\paragraph{Teorema} $Se I in NTIME(f) allora I in TIME(c^f) con c$ $\geq$ $1 che dipende soltanto dalla macchina in NTIME(f). In altre parole\\
se I e' deciso da N a k nastri in tempo non deterministico f(n) allora esiste M a k+1 nastri che decide i in o(c^f(x)), c$ $>$ $1 che dipende solo da N$\\
In altre parole $\textsl{NTIME}(f) \subseteq \textsl{TIME}(c^f)$
\subparagraph{Dim} Grado di diramazione massimo $d$ = max\{grado(q, sigma) = card\{(q, sigma, q', sigma', \} \}\\ %TODO
con archi ordinati, ho una successione di scelte\\
%TODO
...
Se supera il numero di archi la stringa aumenta di un carattere e faccio 00, 01, 02\ldots col numero di archi poi 10, 11, 12\ldots\\
...
\begin{list}{}{}
	\item $N$ decide $I$
	\item $\forall\: x \in I\:\:\:\exists\: t\:|\:M(x)\rightarrow^*\sum$ %TODO
\end{list}
