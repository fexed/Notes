...
Ottimo strumento per dimostrare diversi risultati. Ma l'utilizzo più importante è un altro. Quando scriviamo un programma che deve ordinare un vettore %TODO
\paragraph{Dimostrazione} Dato $i$ trova M$_i$, attraverso una funzione ricorsiva primitiva grazie al teorema di codifica.\\
...
\begin{list}{}{}
	...
	\item e ha il teorema del parametro
\end{list}
%TODO esempio
Supponiamo programma che calcola prodotto con somme successive
\begin{verbatim}
P := 0;
while y > 0
	P := P + w;
...
Operazioni che si possono fare sugli algoritmi che ci lasciano all'interno delle funz calc.\\
$\forall$ $f$ calcolabile totale $\exists$ $n$ $\vline$ $\phi_n = \phi_{f(n)}$. Posso trasformare il mio programma in maniera da ottenerne uno perfettamente equivalente.\\\\\\
$\psi(u, z) = \left\{
\begin{array}{c c}
...
$\{(i, x, k)$ $|$ $\exists\: y,n$ con $(y, n, x < k) \wedge \phi_i(x)\downarrow$ in $n$ passi\}. Limita spazio e tempo\\
$\{(i, x, k, z)$ $|$ $\exists\: n < k \wedge \phi_i(x) = z$ in $n$ passi\}\\
$f(i, x)$ %TODO
$k$? calcolabile totale\\\\
I è \textbf{ricorsivamente enumerabile} $\Leftrightarrow$ è dominio di una funzione calcolabile, cioè $\exists\: i\:\:|\:\: I = \textsl{dom}(\phi_i)$
\begin{list}{}{}
	...
	\textbf{Dim}: se I ric enum esiste $\phi_i$ con domino $I$, e $\overline{I}$ ric enum allora esiste $\phi_{\overline{i}}$. Per sapere se $x$ sta o non sta in $I$, faccio un passo in $i$ se termina sta in $I$, altrimenti un passo nella $\overline{i}$ se termina non sta in $I$ quindi sta in $\overline{I}$. Se non terminano, ripeto, finché non termino.
\end{list}
Ric enum percHé vogliamo enumerare i suoi elementi, tirandone fuori uno alla volta.
\paragraph{Teorema di equivalenza fra caratterizzazioni}
$I$ è ric enm sse I è vuoto oppure I = imm(f), f calc tot.\\
\subparagraph{Dim} Primo caso banale (funz caratt dà sempre 0, ric per per vedi sopra\\
$I \neq \emptyset$, $I = \textsl{dom}(\phi_i)$ costruisco la $f$
%TODO coda di rondine
Indice di riga tiene traccia del numero di passi\\ %TODO
Indice di colonna tiene traccia dell'argomento\\\\ %TODO
\begin{tabular}{c || c | c | c | c | c | c}
	 & 0 & 1 & 2 & 3 & 4 & 5 \\
	 \hline
	 \hline
...
\paragraph{}\textbf{K} ricorsivamente enumerabile? Si. %TODO dim
\subparagraph{Dimostrazione}
\paragraph{}\textbf{K} ricorsivo? No.
\subparagraph{Dimostrazione} Per assurdo, \textbf{K} ricorsivo. Allora $\chi_K(x) =$ 1 se x in K, 0 altrimenti è calcolabile totale.\\
Prendiamo $f(x) = \phi_x(x) + 1$ se $\chi_K(x) = 1$, 0 altriemnti. poiché $\chi$ calc tot, anche $f$ calc tot.\\
Cerchiamo il numero di $f$, ma non lo troviamo. Perché poniamo $i$ l'indice di $f$, allora $\phi_i(i) = f(i) = \phi_(i) + 1$ se $i \in K$, ma non può essere. Se $i \not\in K$ non può essere $\phi_i(i) = f(i) = 0$ perché $\phi_i(i)\uparrow$
\paragraph{Osservazione} \textbf{Ricorsivi} $\subset$ \textbf{ricorisivi enumerabili} $\subset$ non \textbf{ricorsivi enumerabili}\\\\
\textbf{Bootstrapping} $C_L^{L\rightarrow A} (C_L^{L\rightarrow A})= C_A^{L\rightarrow A}$\\
K$_0$ = \{$(x, y)$ $|$ $\phi_y(x)\downarrow$\} problema della fermata (halting). Dato $x$ e $y$, il programma $P_y(x)$ termina? K$_h$ non è ricorsivo. Perché $x \in K \Leftrightarrow (x, x) \in K_0$
\paragraph{Riduzione} $f$ è una \textbf{riduzione} da A a B, si scrive $A \leq_f B$ $\Leftrightarrow (\forall x \in A \Leftarrow f(x) \in B)$\\
...
\paragraph{Classi di problemi} $D, E$ \textbf{classi di problemi} $D \subseteq E (\subseteq$ ricorsivi$)$, allora $\leq_F$ \textbf{classifica} $D$, $E$ $\Leftrightarrow \forall A, B, C$
\begin{enumerate}
	\item $A \leq_F A$\\ Cioè $id \in F$
	\item $A \leq_F B$, $B \leq_F C$ $\Rightarrow A \leq_F C$\\ Cioè $f, g \in F \Rightarrow f(g) \in F$
	\item $A \leq_F B$, $B \in D \Rightarrow A \in D$\\
	\textbf{Preordine parziale}, perché non vale la simmetria. Potrebe A min B, B min A ma non coincidere\\
	D è ideale, o chiuso all'ingiù per riduzione.
	\item $A \leq_F B$, $B \in E \Rightarrow A \in E$\\
	E è ideale, o chiuso all'ingiù per riduzione.
\end{enumerate}
\subsection{Problema Arduo}
H è $\leq_F$-arduo per E $\Leftrightarrow \forall A \in E A \leq_F H$
\subsection{Problema Completo}
C è $\leq_F$-completo per E $\Leftrightarrow C \leq_F$-arduo e $C \in E$
\paragraph{Dim} Sia $\leq_F$ classifica D ed E, allora
\begin{list}{}{}
	\item C $\leq_F$-completo per E, C $\subset$ D $\Leftrightarrow$ D = E
\end{list}
Per ipotesi, $C \in D, A in E$
\end{document}