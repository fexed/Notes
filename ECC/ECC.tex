...
\section{•}
Definiamo un insieme di funzioni $REC = \{\phi_x | \textsl{dom}(\phi_x) = N\}$ insieme di tutte le funzioni calcolabili\\
$TOT = \{x | \phi_x \leq REC\}$\\
$I \leq_{REC} J \Leftrightarrow \exists f \in REC$ $|$ $x \in I \Leftrightarrow f(x) \in J$\\
Per decidere la non appartenenza ad un insieme ricorsivamente enumerabile è necessario un \textbf{tempo infinito}.
\paragraph{Th} $\leq_{REC}$ classifica $\mathscr{R}$ e $\mathscr{RE}$
\subparagraph{Dim} $\mathscr{R} \subseteq \mathscr{RE}$
\begin{enumerate}
	\item $id \in REC$
	\item $f$, $g \in REC \Rightarrow g(f) \in REC$
	\item $B \in \mathscr{R} \Rightarrow A = \{x | f(x) \in B\} \in \mathscr{R}$\\
	Per vedere A in R devo vedere che funz A è funz caratt calc tot\\
	$\chi_A = \chi_B + f$ che è calcolabile totale perché $\chi_B$ è calcolabile totale
	\item \textbf{idem} per $\mathscr{RE}$ con la funzione semi-caratteristica, $\phi_i$ il cui dominino è $B$
\end{enumerate}
Chiaro anche che $A \leq_{REC} B$
\begin{list}{}{}
	\item B è ricorsiva $\Rightarrow$ A è ricorsiva
	\item B è ricorsiva enumerabile $\Rightarrow$ A è ricorsiva enumerabile
	%TODO
\end{list}
...
\paragraph{Oss} K è $\leq_{REC}$-completo per $\mathscr{RE}$
\subparagraph{Dim} $K \in \mathscr{RE}$ ok\\
$\forall A \in \mathscr{RE}$ allora $A \leq_{REC} K$, perché
$A \rightarrow \exists\:\:\psi | A = \textsl{dom}(\phi_i)$ con $\psi$ calcolabile $\Rightarrow \exists\:\:\psi'$ calcolabile $| \psi'(x, y) = \psi(x)$\\
$A = \textsl{dom}(\psi) = \{x | \psi(x)\downarrow\} = \{x | \psi'(x, y)\downarrow\}$ e siccome è calcolabile allora ha indice diciamo $i$, quindi\\
$= \{x | \phi_i(x, y)\downarrow\} = \{x|\phi_{s(i, x)}(y)\downarrow\} = \{x | \phi_{s(i, x)}(s(i, x))\downarrow\}$ con $f(x) = \lambda x.s(i, x)$ e perché $y$ non dipende da $\phi_{s(i, x)}$ e per il teorema del parametro.
...
\paragraph{Lemma} $A$ insieme di indici che rappresentano funzioni $\neq \emptyset$ e $A \neq N$\\
...
$i_0$ sta in $A$ o in $\overline{A}$, e in uno dei due perché $A \neq \emptyset$ e $A \neq N$ quindi $\overline{A}$ "ha posto".\\
Sia $i_0 \in \overline{A}$, dimostro $K \leq_{REC} A$ (viceversa è una dimostrazione analoga) perché sia $i_1 \in A$ allora $\phi_{i_0} \neq \phi_{i_1}$\\ %TODO
$\psi(x, y) = \phi_{i_1}(y)$ se $x \in K$, $\phi_{i_0}(y)$ altrimenti\\
...
\paragraph{Dim} Corollario di prima: A vuoto allora ok, A = N allora F rapprsenta tutte le funz calcolabili quindi A ricorsivo, altrimenti K si riduce ad A o $\overline{A}$ e si riduce alla dim di prima.
\begin{center}
